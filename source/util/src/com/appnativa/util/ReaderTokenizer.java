...
public class ReaderTokenizer {
  ...
  private char[] unicodeChars = new char[4];
  ...
  public int nextToken() throws IOException {
    ...
    byte currentType = (currentChar > 255)
                       ? TOKEN_WORD
      ...
      while((peekOne >= 0) && (peekOne != matchQuote) && (peekOne != '\r') && (peekOne != '\n')) {
        ...
        if (peekOne == '\\') {
          ...
          if ((c1 <= '7') && (c1 >= '0')) {
            ...
            if (n < 4) {
              quoteString.append("u");

              for (int i = 0; i < n; i++) {
                quoteString.append(a[i]);
              }

              peekOne  = c1;
              readPeek = false;
            } else {
              peekOne = CharScanner.unicodeStringToChar(a, 0);
            }
          } else {
            switch(c1) {
              case 'a' :
    ...
    if ((currentChar == '/') && (slashSlashComments || slashStarComments)) {
      if ((currentChar = read()) == '*' && slashStarComments) {
        ...
        while(true) {
          ...
          if (returnComments) {
            comments.append((char) currentChar);
          }
        }
   ...
   * specification.
   */
  private void init_tokinizer() {
    ttype      = TT_UNKNOWN;
    peekChar   = -2;
    lineNumber = 1;
    tokenTypes = new byte[256];
    /*
