...
public final class Helper {
  ...
  private static ThreadLocal<SNumber> perThreadNumber = new ThreadLocal<SNumber>() {
    @Override
    protected synchronized SNumber initialValue() {
  ...
  public static Calendar createCalendar(String spec) {
    ...
    char    ch;
    Boolean pm = null;
    boolean nowMinutes=false;
    int     at   =-1;
    if (len > 1) {
      SNumber snum = perThreadNumber.get();
      at   = sc.indexOf('@');
      ...
      if ((ch == '+') || (ch == '-')) {
        ...
        if (!snum.setValueEx(a, 1, end - 1, true, false)) {
          return null;
        }
      ...
      if (at != -1) {
        ...
        while(sc.currentLen > 0) {
          int pos = sc.currentPos;
          int n   = sc.indexOf(':');

          if (n != -1) {
            if (!snum.setValueEx(a, pos, n - pos, true, false)) {
              return null;
            }

            sc.consume(n - pos + 1);
          } else {
            if (!snum.setValueEx(a, pos, sc.currentLen, true, false)) {
              return null;
            }

            sc.consume(sc.currentLen);
          }
  ...
  public static String tokenReplacement(String s, String what, String with, boolean html, String prefix, String suffix,
    ...
    tokenReplacement(new CharArray(s), what.toCharArray(), with.toCharArray(), html, prefix, suffix, maxLineCount, out);
   ...
   * @param maxLineCount then maximum number of lines to return. If specified and there is more than maxLineCount
   *                     then the string is truncated to maxLineCount and '...' is appended to the end of the string
   * @param out the array that holds the out put
   * @return the converted string
   */
  public static CharArray tokenReplacement(CharArray s, char[] what, char[] with, boolean html, String prefix,
          String suffix, int maxLineCount, CharArray out) {
    int     n = s.indexOf(what, 0);
    int     p = 0;
    int     nn;
    int     tl  = what.length;
    char    c   = what[0];
    ...
    while((lines < maxLineCount) && (n != -1)) {
      ...
      if (html) {
        XMLUtils.escape(a, p, nn, true, out);
      } else {
        out.append(a, p, nn);
      }
    ...
    if ((lines < maxLineCount) && (n > 0)) {
      if (html) {
        XMLUtils.escape(a, p, n, true, out);
      } else {
        out.append(a, p, n);
      }
    }
   ...
   *
   * @return true if date/time was set false otherwise
   */
  @SuppressWarnings("resource")
  public static boolean setDateTime(String val, Calendar cal, boolean settime) {
    int    i    = 0;
    String time = null;
    String date = null;
