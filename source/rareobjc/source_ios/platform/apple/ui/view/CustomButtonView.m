...
- (void)checkForegroundColor {
  ...
  RAREiPaintedButton_ButtonStateEnum *state = [RAREiPaintedButton_ButtonStateEnum DEFAULT];
  RAREUIColor *fg = nil;
  if (!enabled_) {
    state = [self isSelected] ? [RAREiPaintedButton_ButtonStateEnum DISABLED_SELECTED] : [RAREiPaintedButton_ButtonStateEnum DISABLED];
  }
  else {
    if ([self isPressed]) {
      state = [self isSelected] ? [RAREiPaintedButton_ButtonStateEnum PRESSED_SELECTED] : [RAREiPaintedButton_ButtonStateEnum PRESSED];
    }
    else if ([self isSelected]) {
      state = [RAREiPaintedButton_ButtonStateEnum SELECTED];
    }
  }
  if (ph != nil) {
  ...
  if (fg != nil) {
  ...
  else if (foregroundColor_ == [RAREColorUtils getForeground]) {
    if (enabled_) {
    ...
    if (csl != nil) {
      [self setForegroundColorExWithRAREUIColor:[csl getColorWithRAREiPaintedButton_ButtonStateEnum:state]];
    }
  }
}
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _RARECustomButtonView = { "CustomButtonView", "com.appnativa.rare.platform.apple.ui.view", NULL, 0x1, 38, methods, 2, fields, 0, NULL};
