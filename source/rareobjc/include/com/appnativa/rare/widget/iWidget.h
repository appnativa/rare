//
//  Generated by the J2ObjC translator.  DO NOT EDIT!
//  source: /Code/Dev/appNativa/source/rareobjc/../rare/core/com/appnativa/rare/widget/iWidget.java
//
//  Created by decoteaud on 9/15/15.
//

#ifndef _RAREiWidget_H_
#define _RAREiWidget_H_

@class IOSObjectArray;
@class JavaIoWriter;
@class JavaLangThrowable;
@class JavaNetURL;
@class JavaUtilEventObject;
@class RAREActionLink;
@class RAREDragEvent;
@class RAREDropInformation;
@class RAREKeyEvent;
@class RAREMouseEvent;
@class RARERenderTypeEnum;
@class RARESPOTDataItem;
@class RARESPOTWidget;
@class RARETransferFlavor;
@class RAREUIColor;
@class RAREUIDimension;
@class RAREUIFont;
@class RAREUIImage;
@class RAREUIPoint;
@class RAREWidgetContext;
@class RAREWindowViewer;
@class RAREiWidget_WidgetTypeEnum;
@class SPOTPrintableString;
@protocol JavaUtilMap;
@protocol RAREUTiURLResolver;
@protocol RAREiBackgroundPainter;
@protocol RAREiCancelableFuture;
@protocol RAREiContainer;
@protocol RAREiFormViewer;
@protocol RAREiFunctionCallback;
@protocol RAREiPlatformAnimator;
@protocol RAREiPlatformAppContext;
@protocol RAREiPlatformBorder;
@protocol RAREiPlatformComponent;
@protocol RAREiPlatformIcon;
@protocol RAREiPlatformPainter;
@protocol RAREiPopup;
@protocol RAREiScriptHandler;
@protocol RAREiTabDocument;
@protocol RAREiTabPaneViewer;
@protocol RAREiTransferable;
@protocol RAREiViewer;

#import "JreEmulation.h"
#include "com/appnativa/rare/scripting/iScriptingContextSupport.h"
#include "java/lang/Enum.h"

@protocol RAREiWidget < RAREiScriptingContextSupport, NSObject, JavaObject >
- (void)addDataWithJavaUtilMap:(id<JavaUtilMap>)map;
- (void)dispatchKeyEventWithRAREKeyEvent:(RAREKeyEvent *)e;
- (void)dispatchMouseEventWithRAREMouseEvent:(RAREMouseEvent *)e;
- (void)handleKeyboardActionsWithRAREKeyEvent:(RAREKeyEvent *)e;
- (void)setLinkedDataWithId:(id)data;
- (NSString *)getHTTPFormName;
- (id)getHTTPFormValue;
- (id)getLinkedData;
- (BOOL)isDisposed;
- (BOOL)canCopy;
- (BOOL)canCut;
- (BOOL)canDelete;
- (BOOL)canDragWithRAREDragEvent:(RAREDragEvent *)ge;
- (BOOL)canDrop;
- (BOOL)canExportWithRARETransferFlavor:(RARETransferFlavor *)flavor;
- (BOOL)canImportWithRARETransferFlavorArray:(IOSObjectArray *)flavors
                     withRAREDropInformation:(RAREDropInformation *)drop;
- (BOOL)canMove;
- (BOOL)canPaste;
- (BOOL)canRedo;
- (BOOL)canSelectAll;
- (BOOL)canUndo;
- (BOOL)canUndoOrRedo;
- (RAREUIImage *)capture;
- (void)clearContents;
- (void)clearContextData;
- (void)configureWithRARESPOTWidget:(RARESPOTWidget *)cfg;
- (void)dataExportedWithRAREiTransferable:(id<RAREiTransferable>)transferable
                  withRAREDropInformation:(RAREDropInformation *)drop;
- (void)dispose;
- (void)executeEventWithNSString:(NSString *)eventName
         withJavaUtilEventObject:(JavaUtilEventObject *)event;
- (NSString *)expandStringWithNSString:(NSString *)value
                           withBoolean:(BOOL)encode;
- (void)expandStringWithNSString:(NSString *)value
                     withBoolean:(BOOL)encode
                withJavaIoWriter:(JavaIoWriter *)writer;
- (void)finishedLoading;
- (void)fireEventWithNSString:(NSString *)eventName
      withJavaUtilEventObject:(JavaUtilEventObject *)event;
- (void)handleDataURLWithNSString:(NSString *)mimeType
                   withJavaNetURL:(JavaNetURL *)url;
- (void)handleExceptionWithJavaLangThrowable:(JavaLangThrowable *)e;
- (void)hidePopupContainer;
- (BOOL)importDataWithRAREiTransferable:(id<RAREiTransferable>)transferable
                withRAREDropInformation:(RAREDropInformation *)drop;
- (void)reloadWithBoolean:(BOOL)context;
- (id)removeAttributeWithNSString:(NSString *)name;
- (void)removeBackgroundImage;
- (id)removeEventHandlerWithNSString:(NSString *)event
                              withId:(id)code;
- (void)repaint;
- (BOOL)requestFocus;
- (void)reset;
- (void)animateWithRAREiPlatformAnimator:(id<RAREiPlatformAnimator>)animator
               withRAREiFunctionCallback:(id<RAREiFunctionCallback>)cb;
- (void)animateWithNSString:(NSString *)animation
  withRAREiFunctionCallback:(id<RAREiFunctionCallback>)cb;
- (void)selectAll;
- (void)triggerActionEvent;
- (void)triggerActionEventWithNSString:(NSString *)cmd;
- (void)update;
- (void)writeHTTPContentWithBoolean:(BOOL)first
                   withJavaIoWriter:(JavaIoWriter *)writer
                       withNSString:(NSString *)boundary;
- (BOOL)writeHTTPValueWithBoolean:(BOOL)first
                 withJavaIoWriter:(JavaIoWriter *)writer;
- (BOOL)writeJSONValueWithBoolean:(BOOL)first
                 withJavaIoWriter:(JavaIoWriter *)writer;
- (void)setAllowContainerToDisableWithBoolean:(BOOL)allow;
- (void)setAttributeWithNSString:(NSString *)name
                          withId:(id)value;
- (void)setBackgroundWithRAREUIColor:(RAREUIColor *)bg;
- (void)setBackgroundImageWithRAREUIImage:(RAREUIImage *)image
                                  withInt:(int)opacity
                   withRARERenderTypeEnum:(RARERenderTypeEnum *)type;
- (void)setBackgroundOverlayPainterWithRAREiPlatformPainter:(id<RAREiPlatformPainter>)painter;
- (void)setBackgroundPainterWithRAREiBackgroundPainter:(id<RAREiBackgroundPainter>)painter;
- (void)setBorderWithRAREiPlatformBorder:(id<RAREiPlatformBorder>)border;
- (void)setCopyingAllowedWithBoolean:(BOOL)allowed;
- (void)setDataItemsWithRARESPOTDataItem:(RARESPOTDataItem *)items;
- (void)setDataLinkWithRAREActionLink:(RAREActionLink *)link;
- (id<RAREiCancelableFuture>)setDataLinkWithRAREActionLink:(RAREActionLink *)link
                                               withBoolean:(BOOL)defer;
- (void)setDataURLWithNSString:(NSString *)url;
- (void)setDataURLWithJavaNetURL:(JavaNetURL *)url;
- (id<RAREiCancelableFuture>)setDataURLWithNSString:(NSString *)url
                                        withBoolean:(BOOL)defer;
- (void)setDeletingAllowedWithBoolean:(BOOL)allowed;
- (void)setDraggingAllowedWithBoolean:(BOOL)allowed;
- (void)setEnabledWithBoolean:(BOOL)enabled;
- (void)setEventHandlerWithNSString:(NSString *)event
                             withId:(id)code
                        withBoolean:(BOOL)append;
- (void)setFocusPaintedWithBoolean:(BOOL)focusPainted;
- (void)setFocusableWithBoolean:(BOOL)focusable;
- (void)setFontWithRAREUIFont:(RAREUIFont *)font;
- (void)setForegroundWithRAREUIColor:(RAREUIColor *)fg;
- (void)setFromHTTPFormValueWithId:(id)value;
- (void)setIconWithRAREiPlatformIcon:(id<RAREiPlatformIcon>)icon;
- (void)setOpaqueWithBoolean:(BOOL)opaque;
- (void)setOverlayPainterWithRAREiPlatformPainter:(id<RAREiPlatformPainter>)painter;
- (void)setParentWithRAREiContainer:(id<RAREiContainer>)parent;
- (void)setPopupContainerWithRAREiPopup:(id<RAREiPopup>)popup;
- (void)setScriptingContextWithRAREWidgetContext:(RAREWidgetContext *)context;
- (void)setSelectedWithBoolean:(BOOL)selected;
- (void)setSizeLockedWithBoolean:(BOOL)sizeLocked;
- (void)setTextDirectionWithNSString:(NSString *)direction;
- (void)setTitleWithNSString:(NSString *)title;
- (void)setValueWithId:(id)value;
- (void)setVisibleWithBoolean:(BOOL)visible;
- (id<RAREiPlatformAppContext>)getAppContext;
- (id)getAttributeWithNSString:(NSString *)name;
- (RAREUIColor *)getBackground;
- (id<RAREiPlatformPainter>)getBackgroundOverlayPainter;
- (id<RAREiBackgroundPainter>)getBackgroundPainter;
- (id<RAREiPlatformBorder>)getBorder;
- (id<RAREiPlatformComponent>)getContainerComponent;
- (id<RAREiContainer>)getContainerViewer;
- (id<RAREiPlatformComponent>)getDataComponent;
- (JavaNetURL *)getDataURL;
- (id<RAREiPlatformIcon>)getDragIcon;
- (id)getEventHandlerWithNSString:(NSString *)event;
- (IOSObjectArray *)getExportableDataFlavors;
- (RAREUIFont *)getFont;
- (RAREUIColor *)getForeground;
- (id<RAREiFormViewer>)getFormViewer;
- (BOOL)getHasPainterSupport;
- (int)getHeight;
- (id<RAREiPlatformIcon>)getIcon;
- (id<RAREiPlatformIcon>)getIconWithSPOTPrintableString:(SPOTPrintableString *)icon;
- (id<RAREiPlatformIcon>)getIconWithNSString:(NSString *)icon
                                withNSString:(NSString *)description_;
- (id<RAREiPlatformIcon>)getIconWithJavaNetURL:(JavaNetURL *)url
                                  withNSString:(NSString *)description_;
- (RAREUIImage *)getImageWithSPOTPrintableString:(SPOTPrintableString *)image;
- (RAREUIImage *)getImageWithNSString:(NSString *)image;
- (RAREUIImage *)getImageWithJavaNetURL:(JavaNetURL *)url;
- (IOSObjectArray *)getImportableDataFlavors;
- (RAREUIPoint *)getLocationInParent;
- (RAREUIPoint *)getLocationOnScreen;
- (NSString *)getName;
- (id<RAREiPlatformPainter>)getOverlayPainter;
- (id<RAREiContainer>)getParent;
- (NSString *)getPathName;
- (RAREUIDimension *)getPreferredSize;
- (RAREUIDimension *)getPreferredSizeWithInt:(int)maxWidth;
- (int)getScreenLeft;
- (int)getScreenTop;
- (id<RAREiScriptHandler>)getScriptHandler;
- (id)getSelection;
- (NSString *)getSelectionAsString;
- (id)getSelectionData;
- (NSString *)getSelectionDataAsString;
- (id<RAREiTabDocument>)getTabDocument;
- (int)getTabIndex;
- (id<RAREiTabPaneViewer>)getTabPaneViewer;
- (NSString *)getTextDirection;
- (NSString *)getTitle;
- (id<RAREiPlatformComponent>)getTitleLabel;
- (id)getTransferDataWithRARETransferFlavor:(RARETransferFlavor *)flavor
                      withRAREiTransferable:(id<RAREiTransferable>)transferable;
- (id)getUIWindow;
- (JavaNetURL *)getURLWithNSString:(NSString *)url;
- (id<RAREUTiURLResolver>)getURLResolver;
- (id)getValue;
- (NSString *)getValueAsString;
- (id<RAREiViewer>)getViewer;
- (int)getWidgetCount;
- (RAREiWidget_WidgetTypeEnum *)getWidgetType;
- (int)getWidth;
- (RAREWindowViewer *)getWindow;
- (BOOL)hasCustomPropertyWithNSString:(NSString *)name;
- (BOOL)hasPopupMenu;
- (BOOL)hasSelection;
- (BOOL)hasValue;
- (BOOL)isAllowContainerToDisable;
- (BOOL)isDesignMode;
- (BOOL)isEnabled;
- (BOOL)isEventEnabledWithNSString:(NSString *)event;
- (BOOL)isFocusOwner;
- (BOOL)isFocusPainted;
- (BOOL)isInPopup;
- (BOOL)isMultipartContent;
- (BOOL)isRequired;
- (BOOL)isSelectable;
- (BOOL)isSelected;
- (BOOL)isShowing;
- (BOOL)isSizeLocked;
- (BOOL)isSubmittable;
- (BOOL)isTextDirectionSet;
- (BOOL)isTypeWithNSString:(NSString *)type;
- (BOOL)isValidForSubmissionWithBoolean:(BOOL)showerror;
- (BOOL)isViewer;
- (BOOL)isVisible;
@end

#define ComAppnativaRareWidgetIWidget RAREiWidget

typedef enum {
  RAREiWidget_WidgetType_Bean = 0,
  RAREiWidget_WidgetType_PushButton = 1,
  RAREiWidget_WidgetType_Chart = 2,
  RAREiWidget_WidgetType_CheckBox = 3,
  RAREiWidget_WidgetType_CheckBoxTree = 4,
  RAREiWidget_WidgetType_CheckBoxList = 5,
  RAREiWidget_WidgetType_CollapsiblePane = 6,
  RAREiWidget_WidgetType_ColorChooser = 7,
  RAREiWidget_WidgetType_ComboBox = 8,
  RAREiWidget_WidgetType_Custom = 9,
  RAREiWidget_WidgetType_DateChooser = 10,
  RAREiWidget_WidgetType_FileUploadField = 11,
  RAREiWidget_WidgetType_FlashPlayer = 12,
  RAREiWidget_WidgetType_Form = 13,
  RAREiWidget_WidgetType_GridPane = 14,
  RAREiWidget_WidgetType_GroupBox = 15,
  RAREiWidget_WidgetType_WebBrowser = 16,
  RAREiWidget_WidgetType_ImagePane = 17,
  RAREiWidget_WidgetType_Label = 18,
  RAREiWidget_WidgetType_Line = 19,
  RAREiWidget_WidgetType_ListBox = 20,
  RAREiWidget_WidgetType_MenuBar = 21,
  RAREiWidget_WidgetType_Navigator = 22,
  RAREiWidget_WidgetType_PasswordField = 23,
  RAREiWidget_WidgetType_ProgressBar = 24,
  RAREiWidget_WidgetType_RadioButton = 25,
  RAREiWidget_WidgetType_ScrollPane = 26,
  RAREiWidget_WidgetType_Slider = 27,
  RAREiWidget_WidgetType_Spinner = 28,
  RAREiWidget_WidgetType_SplitPane = 29,
  RAREiWidget_WidgetType_StackPane = 30,
  RAREiWidget_WidgetType_StatusBar = 31,
  RAREiWidget_WidgetType_Table = 32,
  RAREiWidget_WidgetType_TabPane = 33,
  RAREiWidget_WidgetType_TextField = 34,
  RAREiWidget_WidgetType_TextArea = 35,
  RAREiWidget_WidgetType_DocumentPane = 36,
  RAREiWidget_WidgetType_ToolBar = 37,
  RAREiWidget_WidgetType_Tree = 38,
  RAREiWidget_WidgetType_WidgetPane = 39,
  RAREiWidget_WidgetType_Window = 40,
  RAREiWidget_WidgetType_MediaPlayer = 41,
} RAREiWidget_WidgetType;

@interface RAREiWidget_WidgetTypeEnum : JavaLangEnum < NSCopying > {
}
+ (RAREiWidget_WidgetTypeEnum *)Bean;
+ (RAREiWidget_WidgetTypeEnum *)PushButton;
+ (RAREiWidget_WidgetTypeEnum *)Chart;
+ (RAREiWidget_WidgetTypeEnum *)CheckBox;
+ (RAREiWidget_WidgetTypeEnum *)CheckBoxTree;
+ (RAREiWidget_WidgetTypeEnum *)CheckBoxList;
+ (RAREiWidget_WidgetTypeEnum *)CollapsiblePane;
+ (RAREiWidget_WidgetTypeEnum *)ColorChooser;
+ (RAREiWidget_WidgetTypeEnum *)ComboBox;
+ (RAREiWidget_WidgetTypeEnum *)Custom;
+ (RAREiWidget_WidgetTypeEnum *)DateChooser;
+ (RAREiWidget_WidgetTypeEnum *)FileUploadField;
+ (RAREiWidget_WidgetTypeEnum *)FlashPlayer;
+ (RAREiWidget_WidgetTypeEnum *)Form;
+ (RAREiWidget_WidgetTypeEnum *)GridPane;
+ (RAREiWidget_WidgetTypeEnum *)GroupBox;
+ (RAREiWidget_WidgetTypeEnum *)WebBrowser;
+ (RAREiWidget_WidgetTypeEnum *)ImagePane;
+ (RAREiWidget_WidgetTypeEnum *)Label;
+ (RAREiWidget_WidgetTypeEnum *)Line;
+ (RAREiWidget_WidgetTypeEnum *)ListBox;
+ (RAREiWidget_WidgetTypeEnum *)MenuBar;
+ (RAREiWidget_WidgetTypeEnum *)Navigator;
+ (RAREiWidget_WidgetTypeEnum *)PasswordField;
+ (RAREiWidget_WidgetTypeEnum *)ProgressBar;
+ (RAREiWidget_WidgetTypeEnum *)RadioButton;
+ (RAREiWidget_WidgetTypeEnum *)ScrollPane;
+ (RAREiWidget_WidgetTypeEnum *)Slider;
+ (RAREiWidget_WidgetTypeEnum *)Spinner;
+ (RAREiWidget_WidgetTypeEnum *)SplitPane;
+ (RAREiWidget_WidgetTypeEnum *)StackPane;
+ (RAREiWidget_WidgetTypeEnum *)StatusBar;
+ (RAREiWidget_WidgetTypeEnum *)Table;
+ (RAREiWidget_WidgetTypeEnum *)TabPane;
+ (RAREiWidget_WidgetTypeEnum *)TextField;
+ (RAREiWidget_WidgetTypeEnum *)TextArea;
+ (RAREiWidget_WidgetTypeEnum *)DocumentPane;
+ (RAREiWidget_WidgetTypeEnum *)ToolBar;
+ (RAREiWidget_WidgetTypeEnum *)Tree;
+ (RAREiWidget_WidgetTypeEnum *)WidgetPane;
+ (RAREiWidget_WidgetTypeEnum *)Window;
+ (RAREiWidget_WidgetTypeEnum *)MediaPlayer;
+ (IOSObjectArray *)values;
+ (RAREiWidget_WidgetTypeEnum *)valueOfWithNSString:(NSString *)name;
- (id)copyWithZone:(NSZone *)zone;
- (id)initWithNSString:(NSString *)__name withInt:(int)__ordinal;
@end

#endif // _RAREiWidget_H_
