...
+ (BOOL)fromJSValueRef:(JSValueRef)value inContext:(JSContextRef)ctx typeEncoding:(char)typeEncoding fullTypeEncoding:(NSString*)fullTypeEncoding fromStorage:(void*)ptr {
    return  [JSCocoaFFIArgument fromJSValueRef:value inContext:ctx typeEncoding:typeEncoding fullTypeEncoding:fullTypeEncoding fromStorage:ptr forFunctionCall:NO];
}

+ (BOOL)fromJSValueRef:(JSValueRef)value inContext:(JSContextRef)ctx typeEncoding:(char)typeEncoding fullTypeEncoding:(NSString*)fullTypeEncoding fromStorage:(void*)ptr  forFunctionCall: (BOOL) forFunctionCall
{
...
//	NSLog(@"toJSValueRef: %c ptr=%p", typeEncoding, ptr);
	switch  (typeEncoding)
	{
      ...
      //RARE: Don DeCoteau, commented the line above and made char convert to boolan
        case	_C_CHR: {
          BOOL b = *(BOOL*)ptr;
...
+ (BOOL)boxObject:(id)objcObject toJSValueRef:(JSValueRef*)value inContext:(JSContextRef)ctx {
	...
	if (!objcObject || objcObject==[NSNull null]) {
		*value = JSValueMakeNull(ctx);
		return	YES;
	}
  ...
  //RARE: Don DeCoteau changed to not retain the boxed objects
	*value = [[JSCocoa controllerFromContext:ctx] boxObjectNoRetain:objcObject];
	return	YES;
}
...
//			NSLog(@"double %f", v);
	...
	JSCocoaPrivateObject* private = JSObjectGetPrivate(jsObject);
    //RARE: Don DeCoteau check for native function and convert so we can map to interfaces
  if(!private && JSObjectIsFunction(ctx, jsObject)) {
      id f= [[[RAREJSFunction alloc] initWithContext:ctx andValue:value] autorelease];
      *o=f;
      return YES;
  }
    ...
    //RARE: Don DeCoteau add custom unboxing
    if(isArray) {
      id f= [[[RAREJSArray alloc] initWithContext:ctx andValue:jsObject] autorelease];
      *o=f;
      return YES;
