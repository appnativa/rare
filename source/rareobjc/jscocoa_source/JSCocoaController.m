...
//
// Init
// RARE: Don DeCoteau - added context group support
- (id)initWithGlobalContext:(JSGlobalContextRef)_ctx useGroup: (JSContextGroupRef)group
{
...
//		jsCocoaObjectDefinition.hasInstance			= jsCocoaObject_hasInstance;
    ...
    ownsContext=YES;
    if(group) {
      ctx=JSGlobalContextCreateInGroup(group,OSXObjectClass);
    }
    else {
	...
	{
		useJSLint		= NO;
    //RARE: Don DeCoteau we want a clean environment so comment the following lines
//		id lintPath = [[NSBundle bundleForClass:[self class]] pathForResource:@"raw/jslint-jscocoa" ofType:@"js"];
...
//		}
//		id classKitPath = [[NSBundle bundleForClass:[self class]] pathForResource:@"raw_apple/class" ofType:@"js"];
...
- (id)init
{
	self = [self initWithGlobalContext:nil useGroup: nil];
  if(self) {
    ownsContext = YES;
  }
	return	self;
...
- (JSValueRef)_toJS:(id)object {
  ...
  else if ([object isKindOfClass:[JavaLangBoolean class]]) { //RARE: Don DeCoteau add conversion for java boolean
    JavaLangBoolean* b	= (JavaLangBoolean*)object;
    return JSValueMakeBoolean(ctx, [b booleanValue]);
  }
...
- (JSValueRef)RAREtoJS:(id)object {
	...
	//NSLog(@"Don't know how to convert %@, boxing it", object);
	return [self boxObjectNoRetain:object];
}
...
//RARE: Don DeCoteau rewrite to remove cache and add support for special RARE* constructs
- (JSObjectRef)boxObject:(id)o
{
  if([o isKindOfClass:[RAREJSFunction class]]) {
    RAREJSFunction* f=(RAREJSFunction*)o;
    return JSValueToObject(self.ctx, f->function, NULL);
...
#pragma mark Boxed object hash
//RARE: Don DeCoteau added to not call when I dont want to retain any boxed objects
- (JSObjectRef)boxObjectNoRetain:(id)o
{
  if([o isKindOfClass:[RAREJSFunction class]]) {
    RAREJSFunction* f=(RAREJSFunction*)o;
    return JSValueToObject(self.ctx, f->function, NULL);
...
//	NSLog(@"QUICK FIX : key is ctx+llx, AND ctx is always the global context");
  id key	= [NSString stringWithFormat:@"%p", o];
	BoxedJSObject* value= nil;//(BoxedJSObject*)[boxedObjects valueForKey:key];
//	
...
//
  ...
  //RARE: Don DeCoteau ,add suport for RAREJSFUnction
  if([o isKindOfClass:[RAREJSFunction class]]) {
    RAREJSFunction* f=(RAREJSFunction*)o;
    return JSValueToObject(f->functionRef.ctx, f->functionRef.value, NULL);
...
JSValueRef valueOfCallback(JSContextRef ctx, JSObjectRef function, JSObjectRef thisObject, size_t argumentCount, const JSValueRef arguments[], JSValueRef *exception)
{
  ...
  //RAREJSFunction special case
  //RARE: Don DeCoteau
  if ([thisPrivateObject.object isKindOfClass:[RAREJSFunction class]]) {
    RAREJSFunction* f=(RAREJSFunction*)thisPrivateObject.object;
    return f->function;
  }
...
call:		
      ...
      //RARE: Don DeCoteau //move this call to happen later as we want to be the fallback not the primary
			// Check if delegate handles getting
...
//					id script = [NSString stringWithFormat:@"return arguments[0].toString().indexOf('[native code]') != -1", propertyName];
          //DON: commented line above and replaced withthe one belwow
          id script = [NSString stringWithFormat:@"return %@.toString().indexOf('[native code]') != -1", propertyName];
					JSStringRef scriptJS = JSStringCreateWithUTF8CString([script UTF8String]);
					JSObjectRef fn = JSObjectMakeFunction(ctx, NULL, 0, NULL, scriptJS, NULL, 1, NULL);
...
//					NSLog(@"isNative(%@)=%d rawJSResult=%p hashProperty=%p returnHashValue=%d", propertyName, isNativeCode, result, hashProperty, returnHashValue);
    ...
    //RARE: Don DeCoteau call the callback after we have already tried other methods to obtain a raw property
    if ([delegate respondsToSelector:@selector(JSCocoa:getProperty:ofObject:inContext:exception:)])
    {
        JSValueRef delegateGet = [delegate JSCocoa:jsc getProperty:propertyName ofObject:privateObject.object inContext:ctx exception:exception];
        if (delegateGet)		return	delegateGet;
...
//				if ([returnValue ffi_type] == &ffi_type_void)	return	NULL;
      ...
      //RARE: Don DeCoteau add class method check
      && ![[callee class] respondsToSelector:NSSelectorFromString(methodName)]
        // non ObjC methods
...
//RARE: Don DeCoteau comment rest of code as we want to fail on a function call
//				//
...
*/
		...
		// Get method pointer
    //RARE: Done Decoteau commented the next 2 lines and replaced with block that comes after the commented line
		...
		Method method;
    if(argumentCount>0 && [((NSString*)methodName) rangeOfString:@":"].location==NSNotFound) {
      method=nil;
    }
    else {
      method=class_getInstanceMethod([callee class], NSSelectorFromString(methodName));
      if (!method)	{
        method = class_getClassMethod([callee class], NSSelectorFromString(methodName));
        //RARE: Don DECoteau added code below
        if(method) {
          callee=[callee class];
        }
      }
    ...
    //RARE:End
        ...
        //RARE: Don DeCoteau add call to delegate to get method
        if(!method && delegate && [delegate respondsToSelector:@selector(JSCocoa:getMethod:ofObject:argumentCount:arguments:inContext:)]) {
          NSString* name=[delegate JSCocoa:jsc getMethod:methodName ofObject:callee argumentCount:argumentCount arguments:arguments inContext:ctx];
          if(name) {
            methodName=name;
            method=class_getInstanceMethod([callee class], NSSelectorFromString(methodName));
            if(!method) {
                if([callee isKindOfClass:[ IOSClass class]]) {
                    callee= ((IOSClass *) callee).objcClass;
                    method=class_getClassMethod(callee, NSSelectorFromString(methodName));
                }
                else {
                    method=class_getClassMethod([callee class], NSSelectorFromString(methodName));
                    if(method) {
        ...
        //RARE: Don DeCoteau add if(!method) around the following block
        if(!method) {
          // Last chance before exception : try calling DO
...
//				NSLog(@"superClass=%@ (old=%@) (%@) function=%p", superSelectorClass, [callee superclass], [callee class], function);
        ...
        switch  (arg.typeEncoding)
        {
            case	_C_ID:
            case	_C_CLASS:
            {
              BOOL okAsIs=NO;
                void** o=arg.rawStoragePointer;
                if([*(id*)o isKindOfClass:[RAREJSFunction class]]) {
                  *(id*)o = [((RAREJSEngine *) delegate) getParamForJSFunction:methodName paramIndex:i objectValue:(RAREJSFunction *) *(id *) o controller: jsc];
                  okAsIs=!([*(id*)o isKindOfClass:[RAREJSFunction class]]);
                }
                if(!okAsIs) {
                  BOOL hadException=NO;
                  if(JSValueIsBoolean(ctx, jsValue)) {
                    if(*o) {
                      *(id*)o=[JavaLangBoolean getTRUE];
                    }
                    else {
                      *(id*)o=[JavaLangBoolean getFALSE];
                    }
                  }
                  else {
                    *(id*)o = [((RAREJSEngine *) delegate) getParam:methodName paramIndex:i objectValue:(NSObject *) *(id *) o hadException:&hadException controller: jsc];
                  }
                  if(hadException) {
                    return	throwException(ctx, exception, [NSString stringWithFormat:@"Argument %c not converted", [arg typeEncoding]]), NULL;
                  }
...
static JSObjectRef jsCocoaObject_callAsConstructor(JSContextRef ctx, JSObjectRef constructor, size_t argumentCount, const JSValueRef arguments[], JSValueRef* exception)
{
    ...
    JSCocoaController* jsc = [JSCocoaController controllerFromContext:ctx];
    id delegate = jsc.delegate;
    if (delegate && [delegate respondsToSelector:@selector(JSCocoa:getMethod:ofObject:argumentCount:arguments:inContext:)]) {
      id object=privateObject.object;
      NSString* methodName=[delegate JSCocoa:jsc getMethod:@"init" ofObject:object argumentCount:argumentCount arguments:(JSValueRef *)arguments inContext:ctx];
      if(methodName) {
        JSValueRef*	argumentsToFree = NULL;
        Class cls;
...
//	JSObjectRef function = [JSCocoaController jsCocoaPrivateFunctionInContext:ctx];
        ...
        // We can get nil when initWith... fails. (eg var image = NSImage.instance({withContentsOfFile:'DOESNOTEXIST'})
        // Return nil then.
        if (returnObject == nil) return throwException(ctx, exception, @"Could not instantiate object"), NULL;
        private = JSObjectGetPrivate(returnObject);
        id boxedObject = [private object];
        [boxedObject release];
        return	returnObject;
    ...
    return throwException(ctx, exception, @"Calling constructor on a non struct"), NULL;
  }
...
void throwException(JSContextRef ctx, JSValueRef* exception, NSString* reason)
{
...
//		if (isSpeaking)	system([[NSString stringWithFormat:@"say \"%@\" &", reason] UTF8String]);
  ...
  //Add a space at the endo of reason
