...
- (int)convertColumnModelIndexToViewWithInt:(int)index {
  return [((id<RAREiTableComponent>) nil_chk(tableHandler_)) convertModelIndexToViewWithInt:index];
}

- (int)convertColumnViewIndexToModelWithInt:(int)index {
  return [((id<RAREiTableComponent>) nil_chk(tableHandler_)) convertViewIndexToModelWithInt:index];
}
...
- (void)setParentItemsSelectableWithBoolean:(BOOL)parentItemsSelectable {
  if (treeHandler_ != nil) {
    [treeHandler_ setParentItemsSelectableWithBoolean:parentItemsSelectable];
  }
}

- (BOOL)isParentItemsSelectable {
  if (treeHandler_ != nil) {
    return [treeHandler_ isParentItemsSelectable];
  }
  return YES;
}
...
- (void)setExpandAllWithBoolean:(BOOL)expandAll {
  if (treeHandler_ != nil) {
    [treeHandler_ setExpandAllWithBoolean:expandAll];
  }
}
...
- (void)configureExWithRARESPOTTable:(RARESPOTTable *)cfg {
  ...
  if ([((SPOTBoolean *) nil_chk(cfg->singleClickActionEnabled_)) spot_valueWasSet]) {
    [comp setSingleClickActionWithBoolean:[cfg->singleClickActionEnabled_ booleanValue]];
  }
  else if ([RAREPlatform isTouchDevice]) {
    [comp setSingleClickActionWithBoolean:YES];
  }
  ...
  tableStyle->headerCellPainter_ = [RAREColorUtils configureWithRAREiWidget:self withRARESPOTGridCell:[cfg getHeaderCell] withRAREPaintBucket:nil];
  tableStyle->headerFillerPainter_ = [RAREColorUtils configureWithRAREiWidget:self withRARESPOTGridCell:[cfg getHeaderRightFillerCell] withRAREPaintBucket:nil];
  ...
  if (treeHandler_ != nil) {
    ...
    [treeHandler_ setParentItemsSelectableWithBoolean:[((SPOTBoolean *) nil_chk(tt->parentItemsSelectable_)) booleanValue]];
    [treeHandler_ setIndentByWithInt:[RAREUIScreen platformPixelsWithInt:[((SPOTInteger *) nil_chk(tt->indentBy_)) intValue]]];
    [treeHandler_ setExpandAllWithBoolean:[((SPOTBoolean *) nil_chk(tt->expandAll_)) booleanValue]];
    [treeHandler_ setShowRootHandlesWithBoolean:[((SPOTBoolean *) nil_chk(tt->showRootHandles_)) booleanValue]];
    if ([((SPOTBoolean *) nil_chk(tt->toggleOnTwistyOnly_)) spot_hasValue]) {
      [treeHandler_ setToggleOnTwistyOnlyWithBoolean:[tt->toggleOnTwistyOnly_ booleanValue]];
    }
    else {
...
- (NSString *)getWidgetAttributeWithNSString:(NSString *)name {
  ...
  if (n != -1) {
    ...
    (void) [sc trim];
    [sc chopWithInt:1];
    name = [sc nextTokenWithChar:'['];
    col = [RAREUTSNumber intValueWithNSString:[self resolvePotentialVariableOrFunctionWithNSString:[sc nextTokenWithChar:',' withBoolean:YES]]];
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _RAREaTableViewer = { "aTableViewer", "com.appnativa.rare.viewer", NULL, 0x401, 72, methods, 7, fields, 0, NULL};
