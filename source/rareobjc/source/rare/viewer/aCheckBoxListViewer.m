...
- (IOSObjectArray *)getCheckedData {
  return [RAREDataItemCollection getValuesWithJavaUtilList:listComponent_ withIntArray:[self getCheckedIndexes] withInt:-1 withBoolean:YES];
}
...
- (void)configureExWithRARESPOTListBox:(RARESPOTListBox *)cfg {
  ...
  if ([((SPOTBoolean *) nil_chk(((RARESPOTCheckBoxList *) nil_chk(cbl))->checkboxTrailing_)) spot_valueWasSet]) {
  ...
  if ([((RAREUIColor *) nil_chk(fg)) isDarkColor]) {
    ...
    indeterminate = [RAREPlatform getResourceAsIconWithNSString:@"Rare.icon.checkbox.indeterminate.dark"];
  }
  else {
    ...
    indeterminate = [RAREPlatform getResourceAsIconWithNSString:@"Rare.icon.checkbox.indeterminate.light"];
  }
