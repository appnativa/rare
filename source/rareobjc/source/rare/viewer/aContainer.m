...
+ (id<RAREiWidget>)createWidgetWithRAREiContainer:(id<RAREiContainer>)parent
  ...
  @try {
    ...
    id<RAREiWidget> w = [RAREaContainer createWidgetWithRAREiContainer:parent withRARESPOTWidget:cfg withJavaNetURL:[link getURLWithRAREUTiURLResolver:parent]];
...
- (void)targetAcquiredWithRAREiTarget:(id<RAREiTarget>)target {
  ...
  [self registerWidgetsWithRAREiFormViewer:fv];
}

- (void)targetLostWithRAREiTarget:(id<RAREiTarget>)target {
  id<RAREiFormViewer> fv = [self getFormViewer];
  [super targetLostWithRAREiTarget:target];
  [self unregisterWidgetsWithRAREiFormViewer:fv];
}
...
- (BOOL)requestFocus {
  ...
  return [self handleFocusWithRAREiWidget:nil withBoolean:YES];
}
...
- (id<RAREiFormViewer>)getFormViewerEx {
  ...
  id<RAREiFormViewer> fv = [super getFormViewerEx];
  return (fv == nil) ? self : ((id) fv);
}
...
- (NSString *)getValueAsString {
  return (_toString_ == nil) ? [super getValueAsString] : [_toString_ sequenceDescription];
}
...
- (BOOL)isFocusableInCurrentState {
  ...
  for (int i = 0; i < len; i++) {
    RAREaWidget *w = (RAREaWidget *) check_class_cast([((id<JavaUtilList>) nil_chk(list)) getWithInt:i], [RAREaWidget class]);
    if ([((RAREaWidget *) nil_chk(w)) isFocusableInCurrentState]) {
      return YES;
    }
  }
...
- (RARESPOTViewer *)checkForURLConfigWithRARESPOTViewer:(RARESPOTViewer *)vcfg {
  if ([((SPOTPrintableString *) nil_chk(((RARESPOTViewer *) nil_chk(vcfg))->dataURL_)) getValue] != nil) {
    @try {
      RAREActionLink *link = [RAREActionLink getActionLinkWithRAREUTiURLResolver:self withSPOTPrintableString:vcfg->dataURL_ withInt:0];
...
- (void)registerWidgetsWithRAREiFormViewer:(id<RAREiFormViewer>)fv {
  ...
  for (id<RAREiWidget> __strong w in nil_chk(list)) {
    ...
    if (fv != self) {
      ...
      if ([(id) w isKindOfClass:[RAREaContainer class]]) {
        RAREaContainer *c = (RAREaContainer *) check_class_cast(w, [RAREaContainer class]);
        if (c->actAsFormViewer_ == NO) {
          [c registerWidgetsWithRAREiFormViewer:fv];
        }
      }
    }
  }
  if ((fv != self) && (fv != nil) && !actAsFormViewer_ && (formWidgets_ != nil)) {
    for (id<RAREiWidget> __strong fw in nil_chk([formWidgets_ values])) {
      (void) [fv registerFormWidgetWithRAREiWidget:fw];
    }
  }
}
...
- (void)unregisterWidgetsWithRAREiFormViewer:(id<RAREiFormViewer>)fv {
  id<JavaUtilList> list = [self getWidgetListEx];
  if (list != nil) {
    for (id<RAREiWidget> __strong w in list) {
      if ([(id) w conformsToProtocol: @protocol(RAREiViewer)]) {
        [((id<RAREiViewer>) check_protocol_cast(w, @protocol(RAREiViewer))) unregisterWithBoolean:NO];
      }
      if ((fv != self) && (fv != nil)) {
        ...
        if ([(id) w isKindOfClass:[RAREaContainer class]]) {
          RAREaContainer *c = (RAREaContainer *) check_class_cast(w, [RAREaContainer class]);
          if (((RAREaContainer *) nil_chk(c))->actAsFormViewer_ == NO) {
            [c unregisterWidgetsWithRAREiFormViewer:fv];
          }
        }
        if (!actAsFormViewer_ && (formWidgets_ != nil)) {
          for (id<RAREiWidget> __strong fw in nil_chk([formWidgets_ values])) {
            [fv unregisterFormWidgetWithRAREiWidget:fw];
          }
        }
      }
    }
...
+ (J2ObjcClassInfo *)__metadata {
  static J2ObjcMethodInfo methods[] = {
    ...
    { "registerWidgetsWithRAREiFormViewer:", NULL, "V", 0x4, NULL },
    { "unregisterWidgetsWithRAREiFormViewer:", NULL, "V", 0x4, NULL },
  ...
  static J2ObjcClassInfo _RAREaContainer = { "aContainer", "com.appnativa.rare.viewer", NULL, 0x401, 56, methods, 3, fields, 0, NULL};
