...
- (id)initWithRAREiContainer:(id<RAREiContainer>)parent {
  if (self = [super initWithRAREiContainer:parent]) {
    autoUnregister_ = YES;
    disposable_ = YES;
    disableBehavior_ = [RAREiViewer_DisableBehaviorEnum DISABLE_WIDGETS];
    autoDispose_ = YES;
    ...
    if (![self isDisposed]) {
      ...
      @try {
        ...
        if ((wl != nil) && [wl isEnabledWithNSString:[RAREiConstants EVENT_DISPOSE]]) {
          (void) [wl evaluateWithNSString:[RAREiConstants EVENT_DISPOSE] withJavaUtilEventObject:[[JavaUtilEventObject alloc] initWithId:self] withBoolean:YES];
        }
      }
...
- (void)register__ {
  registered_ = YES;
  if ((local_ && localSet_) || [self isDesignMode]) {
    return;
  }
  if (!localSet_ && local_) {
  ...
  if (registeredName_ != nil) {
    ...
    [((id<RAREiPlatformWindowManager>) nil_chk([((id<RAREiPlatformAppContext>) nil_chk([self getAppContext])) getWindowManager])) unRegisterViewerWithNSString:registeredName_];
  }
  registeredName_ = s;
  ...
  if (options != nil) {
    ...
    if (o != nil) {
      if ([o isKindOfClass:[RAREUIColor class]]) {
        ...
        bg = [RAREUIColorHelper getBackgroundColorWithNSString:(NSString *) check_class_cast(o, [NSString class])];
      }
    }
...
- (void)targetLostWithRAREiTarget:(id<RAREiTarget>)target {
  ...
  if (p != nil) {
    [((id<RAREiFormViewer>) nil_chk([p getFormViewer])) unregisterFormWidgetWithRAREiWidget:self];
  }
  @try {
    [self unloaded];
  }
...
- (void)unregisterWithBoolean:(BOOL)disposing {
  ...
  if (wm != nil) {
    if (registeredName_ != nil) {
      [wm unRegisterViewerWithNSString:registeredName_];
    }
    registeredName_ = nil;
  }
}
...
- (void)setDisabledColorWithRAREUIColor:(RAREUIColor *)color {
  [((id<RAREiPlatformComponent>) nil_chk([self getContainerComponent])) setDisabledColorWithRAREUIColor:color];
}
...
- (void)setFocusableWithBoolean:(BOOL)focusable {
  ...
  if (nameMap_ != nil) {
    id<JavaUtilIterator> it = [((id<JavaUtilCollection>) nil_chk([nameMap_ values])) iterator];
    id o;
    while ([((id<JavaUtilIterator>) nil_chk(it)) hasNext]) {
      o = [it next];
      if ([o conformsToProtocol: @protocol(RAREiWidget)]) {
        [((id<RAREiWidget>) check_protocol_cast(o, @protocol(RAREiWidget))) setFocusableWithBoolean:focusable];
      }
    }
  }
}
...
- (void)setLinkedDataWithId:(id)data {
  [super setLinkedDataWithId:data];
}
...
- (id<RAREiPlatformAppContext>)getAppContext {
  ...
  id<RAREiContainer> p = [self getParent];
  return (p == nil) ? [RAREPlatform getAppContext] : [p getAppContext];
}
...
- (void)configureExWithRARESPOTViewer:(RARESPOTViewer *)cfg
  ...
  if ([self isDesignMode]) {
    disableBehavior_ = [RAREiViewer_DisableBehaviorEnum DISABLE_BOTH];
    RAREUIColor *c = [((RAREUIProperties *) nil_chk([RAREPlatform getUIDefaults])) getColorWithNSString:@"Rare.disabledBackgroundColor"];
    if (c == nil) {
      c = [RAREColorUtils DISABLED_TRANSPARENT_COLOR];
    }
    ...
    s = [cfg->disableBehavior_ spot_getAttributeWithNSString:@"disableOverlayColor"];
    if (s != nil) {
      RAREUIColor *c = [RAREUIColorHelper getBackgroundColorWithNSString:s];
      if (c != nil) {
        [self setDisabledColorWithRAREUIColor:c];
      }
    }
  ...
  if (s == nil) {
    ...
    s = [NSString stringWithFormat:@"a%@_%@", s, [JavaLangInteger toHexStringWithInt:[self hash]]];
  }
  id<RAREiViewer> v = [self isDesignMode] ? nil : [((id<RAREiPlatformAppContext>) nil_chk([self getAppContext])) getViewerWithNSString:s];
  if ((v != nil) && (v != self)) {
    if ([((id<RAREiPlatformAppContext>) nil_chk([self getAppContext])) isDebugEnabled]) {
      [RAREPlatform ignoreExceptionWithNSString:[RAREUTHelper expandStringWithNSString:[((id<RAREiPlatformAppContext>) nil_chk([self getAppContext])) getResourceAsStringWithNSString:@"Rare.runtime.text.viewerDisposeExisting"] withNSStringArray:[IOSObjectArray arrayWithObjects:(id[]){ s } count:1 type:[IOSClass classWithClass:[NSString class]]]] withJavaLangThrowable:nil];
    }
    if ([v getTarget] != nil) {
      (void) [((id<RAREiTarget>) nil_chk([v getTarget])) removeViewer];
    }
    [v dispose];
  }
  widgetName_ = s;
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _RAREaViewer = { "aViewer", "com.appnativa.rare.viewer", NULL, 0x401, 77, methods, 8, fields, 0, NULL};
  return &_RAREaViewer;
}
