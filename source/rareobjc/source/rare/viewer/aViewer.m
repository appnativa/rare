...
- (void)reloadWithBoolean:(BOOL)context {
  ...
  if (context || (link == nil)) {
    if (widgetDataLink_ != nil) {
      ...
      [self setDataLinkWithRAREActionLink:[[RAREActionLink alloc] initWithRAREUTiURLResolver:[self getParent] withJavaNetURL:sourceURL_]];
      return;
    }
  }
  if (link != nil) {
    ...
    if (t != nil) {
      ...
      [link setContextWithRAREUTiURLResolver:p];
      viewerActionLink_ = nil;
...
- (void)registerOrphanWidgetWithRAREiWidget:(id<RAREiWidget>)widget {
  if (widget != nil) {
    ...
    if ([((RAREUTIdentityArrayList *) nil_chk(orphanWidgets_)) addIfNotPresentWithId:widget]) {
      if (registered_) {
        [widget setParentWithRAREiContainer:[self findParent]];
...
- (void)unregisterOrphanWidgetWithRAREiWidget:(id<RAREiWidget>)widget {
  if ((widget != nil) && (orphanWidgets_ != nil) && [orphanWidgets_ removeWithId:widget]) {
    if (registered_) {
...
- (void)configureExWithRARESPOTViewer:(RARESPOTViewer *)cfg
  ...
  if (![self isDesignMode] && [((SPOTPrintableString *) nil_chk(cfg->scriptURL_)) spot_valueWasSet]) {
    @try {
      ...
      link = [RAREActionLink getActionLinkWithRAREUTiURLResolver:self withSPOTPrintableString:cfg->scriptURL_ withInt:0];
      if ([[cfg->scriptURL_ spot_getLinkedData] isKindOfClass:[RAREUTObjectHolder class]]) {
        ...
        h = [RAREDataParser loadScriptURLWithRAREiWidget:self withRARESPOTViewer:cfg withJavaNetURL:[((RAREActionLink *) nil_chk(link)) getURLWithRAREUTiURLResolver:self]];
      }
      if ([((RAREUTObjectHolder *) nil_chk(h))->value_ isKindOfClass:[JavaLangException class]]) {
        ...
        if (code != nil) {
          ...
          if ([((RAREActionLink *) nil_chk(link)) isInlineURL]) {
            ...
            url = [link getURLWithRAREUTiURLResolver:self];
...
- (void)fireLoadEventWithBoolean:(BOOL)loaded {
  ...
  @try {
    if (loaded) {
      [l loadEventWithJavaUtilEventObject:e];
    }
    else {
      [l unloadEventWithJavaUtilEventObject:e];
    }
  }
