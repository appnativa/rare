...
- (void)unregisterWithBoolean:(BOOL)disposing {
  ...
  [self callaViewerunRegisterWithBoolean:disposing];
  ...
  if ((wm != nil) && targetsRegistered_ && (theTargets_ != nil)) {
    targetsRegistered_ = NO;
    int len = [theTargets_ size];
    id<RAREiTarget> t;
    for (int i = 0; i < len; i++) {
      t = [theTargets_ getWithInt:i];
      [((id<RAREiPlatformWindowManager>) nil_chk([((id<RAREiPlatformAppContext>) nil_chk([self getAppContext])) getWindowManager])) unRegisterTargetWithNSString:[((id<RAREiTarget>) nil_chk(t)) getName]];
      id<RAREiViewer> v = [t getViewer];
      if (v != nil) {
...
- (id<RAREiTarget>)createTargetWithNSString:(NSString *)name
  ...
  if ([((SPOTPrintableString *) nil_chk(region->bgColor_)) getValue] != nil) {
    [RAREUIColorHelper configureBackgroundColorWithRAREiPlatformComponent:container withSPOTPrintableString:region->bgColor_];
  }
...
- (void)viewerCreatedWithRAREiWidget:(id<RAREiWidget>)context
  ...
  if (t != nil) {
    (void) [rv registerNamedItemWithNSString:[((id<RAREiViewer>) nil_chk(viewer)) getName] withId:viewer];
