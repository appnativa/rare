...
- (void)writeHTTPContentWithBoolean:(BOOL)first
  ...
  @try {
    if (widgetType_ != [RAREiWidget_WidgetTypeEnum FileUploadField]) {
      [super writeHTTPContentWithBoolean:first withJavaIoWriter:writer withNSString:boundary];
    }
    else {
      NSString *name = [self getName];
      NSString *file = [self getText];
      file = (file == nil) ? nil : [file trim];
      if ((file != nil) && ([file sequenceLength] > 0) && (name != nil)) {
        JavaIoFile *f = [[JavaIoFile alloc] initWithNSString:file];
        [RAREFormHelper writeFileWithBoolean:first withJavaIoWriter:writer withNSString:boundary withNSString:name withJavaIoFile:f withBoolean:NO];
      }
    }
  }
...
- (BOOL)writeJSONValueWithBoolean:(BOOL)first
                 withJavaIoWriter:(JavaIoWriter *)writer {
  if (widgetType_ != [RAREiWidget_WidgetTypeEnum FileUploadField]) {
    return [super writeJSONValueWithBoolean:first withJavaIoWriter:writer];
  }
  @try {
    ...
    file = (file == nil) ? nil : [file trim];
    if ((file != nil) && ([file sequenceLength] > 0) && (name != nil)) {
      JavaIoFileReader *f = [[JavaIoFileReader alloc] initWithNSString:file];
      if (!first) {
        [((JavaIoWriter *) nil_chk(writer)) writeWithNSString:@",\n\t"];
      }
      JavaIoWriter *w = [RAREJSONHelper getWiterWithJavaIoWriter:writer];
      [RAREJSONHelper encodeWithNSString:name withJavaIoWriter:writer];
      [((JavaIoWriter *) nil_chk(writer)) writeWithNSString:@": \""];
      [RAREUTStreams readerToWriterWithJavaIoReader:f withJavaIoWriter:w withCharArray:nil];
      [((JavaIoWriter *) nil_chk(w)) flush];
      [writer writeWithInt:'"'];
      return YES;
    }
    else {
...
- (void)configureExWithRARESPOTTextField:(RARESPOTTextField *)cfg {
  ...
  if ([cfg isKindOfClass:[RARESPOTPasswordField class]]) {
    ...
    if ([cfg isKindOfClass:[RARESPOTFileUploadField class]]) {
      RARESPOTFileUploadField *fcfg = (RARESPOTFileUploadField *) check_class_cast(cfg, [RARESPOTFileUploadField class]);
      RAREUIComponentPainter *cp = [[RAREUIComponentPainter alloc] init];
      [cp setPainterHolderWithRAREPainterHolder:[RAREPainterUtils createButtonPaintHolder]];
      id<RAREiActionComponent> ac = [RAREPlatformHelper createNakedButtonWithRAREiPlatformComponent:formComponent_ withBoolean:NO withInt:0];
      [((id<RAREiActionComponent>) nil_chk(ac)) setTextWithJavaLangCharSequence:[((SPOTPrintableString *) nil_chk(fcfg->buttonText_)) getValue]];
      [ac setComponentPainterWithRAREiPlatformComponentPainter:cp];
      [ac setBorderWithRAREiPlatformBorder:[RAREBorderUtils getButtonBorderWithInt:[RAREPlatform isTouchDevice] ? 6 : 2]];
      s = [((SPOTPrintableString *) nil_chk(fcfg->buttonIcon_)) getValue];
      if (s != nil) {
        id<RAREiPlatformIcon> icon = [self getIconWithSPOTPrintableString:fcfg->buttonIcon_];
        if (icon != nil) {
          [ac setIconWithRAREiPlatformIcon:icon];
        }
      }
      NSString *title = [((SPOTPrintableString *) nil_chk(fcfg->dialogTitle_)) getValue];
      NSString *exts = [((SPOTPrintableString *) nil_chk(fcfg->fileExtensions_)) getValue];
      [ac addActionListenerWithRAREiActionListener:[[RAREaTextFieldWidget_$2 alloc] initWithRAREaTextFieldWidget:self withNSString:title withNSString:exts]];
      RAREBorderPanel *bp = [[RAREBorderPanel alloc] initWithRAREiWidget:self];
      [bp setPaddingWithRAREUIInsets:[[RAREUIInsets alloc] initWithInt:0 withInt:5 withInt:0 withInt:0]];
      [bp setRightViewWithRAREiPlatformComponent:ac];
      [bp setCenterViewWithRAREiPlatformComponent:formComponent_];
      fileUploadButton_ = ac;
      formComponent_ = bp;
      [self registerWithWidgetWithRAREiPlatformComponent:bp];
      [self registerWithWidgetWithRAREiPlatformComponent:ac];
    }
...
- (void)focusEventWithRAREFocusEvent:(RAREFocusEvent *)e {
  ...
  if ([((RAREFocusEvent *) nil_chk(e)) getID] == RAREFocusEvent_FOCUS_LOST) {
    [self verify];
    if (hadError_) {
      hadError_ = NO;
...
- (void)showError {
  hadError_ = YES;
  NSString *title = [((id<RAREiPlatformAppContext>) nil_chk([self getAppContext])) getResourceAsStringWithNSString:@"Rare.runtime.text.fieldInvalidFormatTitle"];
  ...
  if (s != nil) {
    ...
    if (w != nil) {
      ...
      [((id<RAREiPlatformAppContext>) nil_chk([self getAppContext])) oneLineErrorMessageWithNSString:title withNSString:s];
    }
  }
}
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _RAREaTextFieldWidget = { "aTextFieldWidget", "com.appnativa.rare.widget", NULL, 0x401, 36, methods, 20, fields, 0, NULL};
...
- (BOOL)textChangingWithId:(id)source
  ...
  if (([this$0_ getValidCharacters] != nil) && ![RAREUtils isInValidSetWithNSString:[this$0_ getValidCharacters] withJavaLangCharSequence:replacementString withBoolean:NO]) {
    return NO;
  }
  return YES;
}
...
@end
@implementation RAREaTextFieldWidget_$2

- (void)actionPerformedWithRAREActionEvent:(RAREActionEvent *)e {
  JavaIoFile *f = [RAREaPlatformHelper getFileWithRAREiPlatformComponent:this$0_->formComponent_ withNSString:val$title_ withBoolean:YES withBoolean:NO withJavaIoFile:nil withNSString:val$exts_];
  if (f != nil) {
    [this$0_ setValueWithId:[f getAbsolutePath]];
  }
}
...
+ (J2ObjcClassInfo *)__metadata {
  static J2ObjcFieldInfo fields[] = {
    { "this$0_", NULL, 0x1012, "LRAREaTextFieldWidget" },
    { "val$title_", NULL, 0x1012, "LNSString" },
    { "val$exts_", NULL, 0x1012, "LNSString" },
  };
  static J2ObjcClassInfo _RAREaTextFieldWidget_$2 = { "$2", "com.appnativa.rare.widget", "aTextFieldWidget", 0x8000, 0, NULL, 3, fields, 0, NULL};
  return &_RAREaTextFieldWidget_$2;
}
