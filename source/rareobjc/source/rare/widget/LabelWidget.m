...
- (id<RAREiActionComponent>)createActionComponentWithRARESPOTLabel:(RARESPOTLabel *)cfg {
  RARELabelView *label = [((id<RAREiPlatformComponentFactory>) nil_chk([((id<RAREiPlatformAppContext>) nil_chk([self getAppContext])) getComponentCreator])) getLabelWithRAREiWidget:self withRARESPOTLabel:cfg];
  if ([((SPOTBoolean *) nil_chk(((RARESPOTLabel *) nil_chk(cfg))->supportHyperLinks_)) booleanValue]) {
    [((RARELabelView *) nil_chk(label)) setHyperlinkListenerWithRAREiHyperlinkListener:[self createHyperlinkListener]];
  }
  return [[RAREActionComponent alloc] initWithRAREView:label];
}
