...
- (void)addWindowListenerWithRAREiWindowListener:(id<RAREiWindowListener>)l {
  if (listenerList_ == nil) {
    ...
    id<RAREiWindowListener> wl = [[RAREaWindow_$1 alloc] initWithRAREaWindow:self];
    [self setWindowListenerWithRAREiWindowListener:wl];
  }
  [((RAREEventListenerList *) nil_chk(listenerList_)) addWithIOSClass:[IOSClass classWithProtocol:@protocol(RAREiWindowListener)] withId:l];
}
...
- (void)pack {
  ...
  size->height_ += [self getTitlebarHeight];
...
- (void)setToolBarWithRAREiToolBarHolder:(id<RAREiToolBarHolder>)tb {
  [((RAREWindowPane *) nil_chk(rootPane_)) setToolBarWithRAREiToolBarHolder:tb];
}
...
- (void)createTarget {
  NSString *name = [NSString stringWithFormat:@"_new_window_%@", [JavaLangInteger toHexStringWithInt:[self hash]]];
...
- (void)setWindowVisibleWithBoolean:(BOOL)visible {
  ...
  if (visible) {
    [((RAREUTIdentityArrayList *) nil_chk([((id<RAREiPlatformAppContext>) nil_chk([RAREPlatform getAppContext])) getActiveWindows])) addWithId:self];
  }
  else {
...
+ (J2ObjcClassInfo *)__metadata {
  static J2ObjcMethodInfo methods[] = {
    ...
    { "getTitlebarHeight", NULL, "F", 0x4, NULL },
  ...
  static J2ObjcClassInfo _RAREaWindow = { "aWindow", "com.appnativa.rare.platform.apple.ui.view", NULL, 0x401, 50, methods, 10, fields, 0, NULL};
  return &_RAREaWindow;
}
...
- (void)windowEventWithRAREWindowEvent:(RAREWindowEvent *)e {
  [RAREUtils fireWindowEventWithRAREEventListenerList:this$0_->listenerList_ withRAREWindowEvent:e];
}
