...
+ (RAREUIImage *)createImageWithRAREiWidget:(id<RAREiWidget>)context
                         withRAREActionLink:(RAREActionLink *)link
                                withBoolean:(BOOL)defer
                                  withFloat:(float)density {
  return [RAREImageHelper createImageWithJavaNetURL:[((RAREActionLink *) nil_chk(link)) getURLWithRAREUTiURLResolver:context] withBoolean:defer withFloat:density];
}
...
+ (RAREUIImage *)createImageWithJavaNetURL:(JavaNetURL *)url
  ...
  if ([((NSString *) nil_chk(protocol)) isEqual:@"file"] || [protocol isEqual:@"data"]) {
    defer = NO;
  }
  if (density == 0) {
  ...
  JavaLangInteger *to = (JavaLangInteger *) check_class_cast([((RAREUIProperties *) nil_chk([((id<RAREiPlatformAppContext>) nil_chk(app)) getUIDefaults])) getWithNSString:@"Rare.net.imageLoadTimeout"], [JavaLangInteger class]);
  ...
  RAREUIImage *img = (proxy == nil) ? nil : [[RAREUIImage alloc] initWithId:proxy withNSString:[RAREJavaURLConnection toExternalFormWithJavaNetURL:url]];
  if ((img != nil) && (constraints > 0) && (size > 0)) {
  ...
  return img == nil ? [RAREaUIImageIcon getBrokenImage] : img;
}
...
+ (J2ObjcClassInfo *)__metadata {
  static J2ObjcMethodInfo methods[] = {
    ...
    { "load__", NULL, "V", 0x1, "JavaLangException" },
  ...
  static J2ObjcClassInfo _RAREImageHelper_DelayedImage = { "DelayedImage", "com.appnativa.rare.platform.apple.ui.util", "ImageHelper", 0x9, 8, methods, 3, fields, 0, NULL};
