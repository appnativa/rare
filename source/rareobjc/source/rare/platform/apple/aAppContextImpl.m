...
- (void)lockOrientationWithJavaLangBoolean:(JavaLangBoolean *)landscape {
  [RAREPlatformHelper lockOrientationWithJavaLangBoolean:landscape];
}
...
- (RAREUIImage *)getManagedResourceWithNSString:(NSString *)name {
  ...
  if ([name hasPrefix:@"Rare.icon."]) {
    return [super getManagedResourceWithNSString:name];
  }
  if (!managedResourcePathInitialized_) {
    ...
    if ((int) [((IOSObjectArray *) nil_chk(managedResourcePaths_)) count] == 0) {
      useIOSResourceNamingConvention_ = YES;
    }
  }
  if (!useIOSResourceNamingConvention_) {
    return [super getManagedResourceWithNSString:name];
  }
  RAREUIImage *img = nil;
  IOSObjectArray *path = [IOSObjectArray arrayWithLength:5 type:[IOSClass classWithClass:[NSString class]]];
  int len = 0;
  float density = [RAREaPlatformHelper getUnscaledImageDensity];
  RAREUTCharArray *ca = [[RAREUTCharArray alloc] init];
  (void) [ca setWithNSString:[self getDefaultManagedResourcePath]];
  (void) [ca appendWithChar:'/'];
  (void) [ca appendWithNSString:name];
  int dlen = ca->_length_;
  if (density == 2) {
    (void) [ca appendWithNSString:landscapeMode_ ? @"-Landscape@2x" : @"-Portrait@2x"];
    (void) IOSObjectArray_Set(path, len++, [ca description]);
    ca->_length_ = dlen;
    (void) [ca appendWithNSString:@"@2x"];
    (void) IOSObjectArray_Set(path, len++, [ca description]);
    ca->_length_ = dlen;
  }
  (void) [ca appendWithNSString:landscapeMode_ ? @"-Landscape" : @"-Portrait"];
  (void) IOSObjectArray_Set(path, len++, [ca description]);
  ca->_length_ = dlen;
  for (int i = 0; i < len; i++) {
    NSString *rname = IOSObjectArray_Get(path, i);
    JavaNetURL *url = [self checkForFileWithNSString:rname];
    if (url != nil) {
      float fd = 1;
      if ([((NSString *) nil_chk(rname)) contains:@"@2x"]) {
        fd = 2;
      }
      @try {
...
- (RAREUIImage *)getResourceAsImageWithNSString:(NSString *)name {
  RAREUIImageIcon *icon = [self getResourceAsIconWithNSString:name];
  if (icon != nil) {
...
- (void)setupUIDefaults {
  ...
  (void) [((id<JavaUtilMap>) nil_chk(appIcons_)) putWithId:@"Rare.Tree.collapsedIcon" withId:[self getResourceAsIconWithNSString:@"Rare.icon.collapsed"]];
  (void) [appIcons_ putWithId:@"Rare.Tree.expandedIcon" withId:[self getResourceAsIconWithNSString:@"Rare.icon.expanded"]];
}
...
- (void)copyAllFieldsTo:(RAREaAppContextImpl *)other {
  ...
  other->useIOSResourceNamingConvention_ = useIOSResourceNamingConvention_;
}
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcFieldInfo fields[] = {
    ...
    { "useIOSResourceNamingConvention_", NULL, 0x4, "Z" },
  };
  static J2ObjcClassInfo _RAREaAppContextImpl = { "aAppContextImpl", "com.appnativa.rare.platform.apple", NULL, 0x401, 32, methods, 3, fields, 0, NULL};
