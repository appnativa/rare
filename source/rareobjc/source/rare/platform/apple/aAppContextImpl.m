...
- (RAREUIImage *)getManagedResourceWithNSString:(NSString *)name {
  ...
  NSString *oname = name;
  if ([name indexOf:'.'] != -1) {
    RAREUTCharArray *ca = [[RAREUTCharArray alloc] initWithNSString:name];
    (void) [((RAREUTCharArray *) nil_chk([ca toLowerCase])) replaceWithChar:'.' withChar:'_'];
    name = [ca description];
  }
  id proxy = [RAREImageUtils loadAssetCatalogImageProxyWithNSString:name];
  return proxy == nil ? nil : [[RAREUIImage alloc] initWithId:proxy withNSString:oname];
}
