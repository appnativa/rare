...
+ (NSString *)colorToRGBStringWithRAREUIColor:(RAREUIColor *)c {
  ...
  int a = [((RAREUIColor *) nil_chk(c)) getAlpha];
  if (a == 255) {
    return [NSString stringWithFormat:@"%d,%d, %d", [c getRed], [c getGreen], [c getBlue]];
  }
  else {
    return [NSString stringWithFormat:@"%d,%d, %d, %d", [c getRed], [c getGreen], [c getBlue], a];
  }
}
...
+ (id)createContextWithNSString:(NSString *)format
  ...
  @try {
    ...
    if (date) {
      ...
      RAREDateContext *dc = [[RAREDateContext alloc] initWithJavaTextDateFormatArray:a withJavaTextDateFormat:ddf];
      return dc;
    }
    else {
      IOSObjectArray *a = nil;
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _RAREConversions = { "Conversions", "com.appnativa.rare.converters", NULL, 0x1, 19, methods, 1, fields, 0, NULL};
