...
+ (JavaNetURL *)createInlineURLWithNSString:(NSString *)data
  ...
  NSString *s;
  if (enc != nil) {
    s = [NSString stringWithFormat:@"data:%@;%@,%@", mimeType, enc, data];
  }
  else {
    s = [NSString stringWithFormat:@"data:%@,%@", mimeType, data];
  }
  return [[JavaNetURL alloc] initWithNSString:@"http" withNSString:[RAREiConstants INLINE_PROTOCOL_STRING] withNSString:[RAREFunctions encodeWithNSString:s]];
}
...
- (NSString *)toExternalFormWithJavaNetURL:(JavaNetURL *)url {
  if (toString__ == nil) {
    if (encoding_ != nil) {
      toString__ = [NSString stringWithFormat:@"data:%@;%@,%@", mimeType_, encoding_, inlineData_];
    }
    else {
      toString__ = [NSString stringWithFormat:@"data:%@,%@", mimeType_, inlineData_];
    }
  }
  return toString__;
}
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcFieldInfo fields[] = {
    ...
    { "toString__", "toString", 0x0, "LNSString" },
  };
  static J2ObjcClassInfo _RAREaNetHelper_InlineStreamHandler = { "InlineStreamHandler", "com.appnativa.rare.net", "aNetHelper", 0x8, 10, methods, 4, fields, 0, NULL};
