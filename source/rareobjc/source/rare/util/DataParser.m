...
+ (JavaLangRuntimeException *)invalidConfigurationExceptionWithRAREiPlatformAppContext:(id<RAREiPlatformAppContext>)app
  ...
  if (link != nil) {
    @try {
      JavaNetURL *u = [link getURLWithRAREUTiURLResolver:nil];
      if (u != nil) {
        s = [u description];
      }
      else {
...
+ (id<iSPOTElement>)loadSPOTObjectSDFWithRAREiWidget:(id<RAREiWidget>)context
  ...
  @try {
    if (RAREDataParser_DISABLE_ALL_INLINING_OF_URLS_ || dm) {
      root = [SDFNode parseForReformatWithJavaIoReader:reader withRAREUTiURLResolver:ur withNSString:(contextURL == nil) ? nil : [contextURL description]];
    }
    else {
      root = [SDFNode parseWithJavaIoReader:reader withRAREUTiURLResolver:ur withNSString:(contextURL == nil) ? nil : [contextURL description] withBoolean:dm];
    }
  }
...
+ (SPOTSet *)loadSPOTSetWithRAREiWidget:(id<RAREiWidget>)ctx
  ...
  RAREActionLink *link = [[RAREActionLink alloc] initWithRAREUTiURLResolver:ctx withSPOTPrintableString:url];
  SPOTSet *set = [[SPOTSet alloc] initWithNSString:@"Set" withIOSClass:cls];
  @try {
...
+ (RAREUTObjectHolder *)loadScriptURLWithRAREiWidget:(id<RAREiWidget>)context
  ...
  if ([((SPOTPrintableString *) nil_chk(((RARESPOTViewer *) nil_chk(cfg))->scriptURL_)) getValue] != nil) {
    ...
    id<RAREiPlatformAppContext> app = [((id<RAREiWidget>) nil_chk(context)) getAppContext];
    RAREActionLink *link = [[RAREActionLink alloc] initWithRAREUTiURLResolver:context withSPOTPrintableString:cfg->scriptURL_];
...
+ (id<iSPOTElement>)checkForConfigURLWithRAREiWidget:(id<RAREiWidget>)context
  ...
  if ([((SPOTPrintableString *) nil_chk(((RARESPOTGroupBox *) nil_chk(vcfg))->dataURL_)) getValue] != nil) {
    RAREActionLink *link = [[RAREActionLink alloc] initWithRAREUTiURLResolver:context withSPOTPrintableString:vcfg->dataURL_];
    @try {
      [link setContextURLWithJavaNetURL:contextURL];
      ...
      id<JavaUtilMap> map = [vcfg spot_getAttributesEx];
      if (map != nil) {
        [cfg spot_addAttributesWithJavaUtilMap:map];
      }
      vcfg = cfg;
    }
    @catch (JavaLangException *e) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    if ([cfg isKindOfClass:[RARESPOTGroupBox class]]) {
      ...
      if ([@"false" isEqual:[r->dataURL_ spot_getAttributeWithNSString:@"deferred"]] || inline_) {
        RAREActionLink *link = [[RAREActionLink alloc] initWithRAREUTiURLResolver:context withSPOTPrintableString:r->dataURL_];
        @try {
          [link setContextURLWithJavaNetURL:contextURL];
...
+ (id<iSPOTElement>)checkForConfigURLWithRAREiWidget:(id<RAREiWidget>)context
  ...
  @try {
    for (int i = 0; i < len; i++) {
      ...
      RAREActionLink *link = [[RAREActionLink alloc] initWithRAREUTiURLResolver:context withSPOTPrintableString:ps];
      @try {
        [link setContextURLWithJavaNetURL:contextURL];
...
+ (id<iSPOTElement>)loadElementWithRAREiWidget:(id<RAREiWidget>)context
                                withJavaNetURL:(JavaNetURL *)contextURL
                                  withNSString:(NSString *)url {
  @try {
    RAREActionLink *link = [[RAREActionLink alloc] initWithRAREUTiURLResolver:context withNSString:url withNSString:nil];
    @try {
      [link setContextURLWithJavaNetURL:contextURL];
