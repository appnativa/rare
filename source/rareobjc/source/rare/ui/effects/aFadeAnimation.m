...
- (void)clear {
  if (inComponent_ != nil) {
    ...
    [component_ setAlphaWithFloat:1];
  }
  outComponent_ = nil;
  outComponent_ = nil;
}
...
- (void)updateWithFloat:(float)value {
  ...
  if ((inComponent_ != nil) && (outComponent_ != nil)) {
    if (![inComponent_ setAlphaWithFloat:value]) {
      [self cancel];
      return;
    }
    [inComponent_ repaint];
    if ([outComponent_ setAlphaWithFloat:endingAlpha_ - value]) {
      [outComponent_ repaint];
    }
  }
  else {
    if (direction_ == [RAREiAnimator_DirectionEnum FORWARD]) {
      [((id<RAREiPlatformComponent>) nil_chk(component_)) setAlphaWithFloat:value];
    }
    else {
      [((id<RAREiPlatformComponent>) nil_chk(component_)) setAlphaWithFloat:endingAlpha_ - value];
    }
