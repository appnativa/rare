...
- (BOOL)isEditing {
  return [((id<RAREiListView>) nil_chk(listView_)) isEditing];
}
...
- (void)clearPopupMenuIndex {
  [((id<RAREiListView>) nil_chk(listView_)) clearPopupMenuIndex];
}
...
+ (BOOL)focusEventWithRAREiListHandler:(id<RAREiListHandler>)listComponent
  ...
  if ([((RAREFocusEvent *) nil_chk(e)) getID] == RAREFocusEvent_FOCUS_GAINED) {
...
- (int)getPopupMenuIndex {
  return [((id<RAREiListView>) nil_chk(listView_)) getPopupMenuIndex];
}

- (RARERenderableDataItem *)getPopupMenuItem {
  int n = [self getPopupMenuIndex];
...
- (int)getSelectedIndexCount {
  return ([self getSelectedIndex] == -1) ? 0 : 1;
}

- (IOSIntArray *)getSelectedIndexes {
  int n = [self getSelectedIndex];
  return (n == -1) ? [IOSIntArray arrayWithLength:0] : [IOSIntArray arrayWithInts:(int[]){ n } count:1];
}
