...
- (void)setMenuWithRAREUIPopupMenu:(RAREUIPopupMenu *)pm {
  if (showButtons_) {
    ...
    for (int i = 0; i < len; i++) {
      id<RAREiActionComponent> c = [self createButtonWithRAREUIMenuItem:[((RAREUIPopupMenu *) nil_chk(popupMenu_)) getMenuItemWithInt:i]];
      [((JavaUtilArrayList *) nil_chk(buttons_)) addWithId:c];
      if (i > 1) {
        [((JavaUtilArrayList *) nil_chk(separators_)) addWithId:[self createSeparator]];
      }
...
- (void)setTitleActionWithRAREUIAction:(RAREUIAction *)a {
  [((id<RAREiActionComponent>) nil_chk(titleComponent_)) setActionWithRAREUIAction:a];
}
