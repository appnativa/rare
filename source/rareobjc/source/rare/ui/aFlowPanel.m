...
- (void)addWithRAREiPlatformComponent:(id<RAREiPlatformComponent>)c
  ...
  if (!([constraints isKindOfClass:[RARECellConstraints class]])) {
    constraints = [[RARECellConstraints alloc] init];
  }
...
- (void)layoutWithFloat:(float)width
  ...
  for (int i = 0; i < len; i++) {
    ...
    RARECellConstraints *cc = (RARECellConstraints *) check_class_cast([c getClientPropertyWithNSString:[RAREiConstants RARE_CONSTRAINTS_PROPERTY]], [RARECellConstraints class]);
    w = ((RARECellConstraints *) nil_chk(cc))->gridWidth_;
    if (x + w > width) {
...
- (void)layoutWithJavaUtilArrayList:(JavaUtilArrayList *)list
  ...
  for (int i = 0; i < len; i++) {
    ...
    RARECellConstraints *cc = (RARECellConstraints *) check_class_cast([((id<RAREiPlatformComponent>) nil_chk(c)) getClientPropertyWithNSString:[RAREiConstants RARE_CONSTRAINTS_PROPERTY]], [RARECellConstraints class]);
    h = ((RARECellConstraints *) nil_chk(cc))->gridHeight_;
    if (h > height) {
      h = height;
    }
    switch ([((RARECellConstraints_Alignment *) nil_chk(cc->vAlign_)) abbreviation]) {
      case 't':
      yy = y;
      break;
      case 'c':
...
- (void)populateSizeCache {
  ...
  for (int i = 0; i < len; i++) {
    ...
    ((RARECellConstraints *) nil_chk(cc))->gridHeight_ = (int) size->height_;
...
- (void)getPreferredSizeExWithRAREUIDimension:(RAREUIDimension *)size
  ...
  if (maxWidth > 0) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      RARECellConstraints *cc = [RAREaComponent getMeasuredCellConstraintsWithRAREiPlatformComponent:c withBoolean:YES];
      width += ((RARECellConstraints *) nil_chk(cc))->gridWidth_ + columnSpacing_;
...
- (void)getSizeForWidthWithRAREUIDimension:(RAREUIDimension *)size
  ...
  for (int i = 0; i < len; i++) {
    ...
    RARECellConstraints *cc = [RAREaComponent getMeasuredCellConstraintsWithRAREiPlatformComponent:c withBoolean:YES];
    width += ((RARECellConstraints *) nil_chk(cc))->gridWidth_;
    if (width > maxWidth) {
