...
- (id)initWithRAREAppContext:(RAREAppContext *)app {
  if (self = [super initWithRAREiPlatformAppContext:app]) {
    ...
    mainFrame_ = [[RAREFrame alloc] initWithRAREiPlatformAppContext:app withRAREWindow:[((RAREAppContext *) nil_chk(app)) getPlatformMainWindow] withRAREaWindowManager_WindowTypeEnum:[RAREaWindowManager_WindowTypeEnum FRAME]];
...
- (void)configureWithRARESPOTMainWindow:(RARESPOTMainWindow *)cfg {
  ...
  BOOL decorated = YES;
  if ([((id<RAREiPlatformAppContext>) nil_chk([RAREPlatform getAppContext])) okForOSWithISPOTElement:((RARESPOTMainWindow *) nil_chk(cfg))->decorated_]) {
    decorated = [((SPOTBoolean *) nil_chk(cfg->decorated_)) booleanValue];
  }
  [((RAREWindow *) check_class_cast(((RAREFrame *) nil_chk(frame))->window_, [RAREWindow class])) setDecoratedWithBoolean:decorated];
  NSString *s = [((SPOTBoolean *) nil_chk(cfg->decorated_)) spot_getAttributeWithNSString:@"color"];
  RAREUIColor *bg = (s == nil) ? [RAREColorUtils getBackground] : [RAREColorUtils getColorWithNSString:s];
  if ([bg getAlpha] != 0) {
    [((RAREWindow *) check_class_cast(frame->window_, [RAREWindow class])) setBackgroundColorExWithRAREUIColor:bg];
  }
  id<RAREiWidget> tw = [self createTitleWidgetWithRARESPOTMainWindow:cfg];
  if (tw != nil) {
    [frame setTitleWidgetWithRAREiWidget:tw];
  }
  RARESPOTRectangle *r = cfg->bounds_;
  int x = [((RARESPOTRectangle *) nil_chk(r)) getXPixels];
  int y = [r getYPixels];
  int w = [r getWidthPixelsWithRAREiPlatformComponent:frame withBoolean:YES];
  int h = [r getHeightPixelsWithRAREiPlatformComponent:frame withBoolean:YES];
  if (![RAREPlatform isIOS]) {
    ...
    if ([((SPOTBoolean *) nil_chk(cfg->showTitleBar_)) spot_hasValue] && [cfg->showTitleBar_ booleanValue] && [((id<RAREiPlatformAppContext>) nil_chk([RAREPlatform getAppContext])) okForOSWithISPOTElement:cfg->showTitleBar_] && (menuBar_ == nil)) {
      [((RAREWindow *) check_class_cast(frame->window_, [RAREWindow class])) setDecoratedWithBoolean:YES];
    }
  }
...
- (id<RAREiWindowManager_iFrame>)createFrameWithRAREiWidget:(id<RAREiWidget>)context
  ...
  switch ([type ordinal]) {
    case RAREaWindowManager_WindowType_DIALOG:
    win = [RAREaPlatformHelper createDialogWithRAREWindow:[((RAREView *) nil_chk([((id<RAREiPlatformComponent>) nil_chk([((id<RAREiWidget>) nil_chk(context)) getDataComponent])) getView])) getWindow] withBoolean:trans withBoolean:!undecorated withBoolean:modal];
    break;
    case RAREaWindowManager_WindowType_POPUP:
    win = [RAREaPlatformHelper createPopupWithBoolean:YES withBoolean:trans];
    break;
    case RAREaWindowManager_WindowType_POPUP_ORPHAN:
    win = [RAREaPlatformHelper createPopupWithBoolean:NO withBoolean:trans];
    break;
    default:
    win = [RAREPlatformHelper createWindowWithBoolean:modal withBoolean:trans withBoolean:!undecorated];
    break;
  }
  if (trans) {
    [((RAREWindow *) nil_chk(win)) setBackgroundColorWithRAREUIColor:[RAREUIColor TRANSPARENT]];
  }
  RAREFrame *frame = [[RAREFrame alloc] initWithRAREiPlatformAppContext:appContext_ withRAREWindow:win withNSString:nil withRAREaWindowManager_WindowTypeEnum:type];
  frame->undecorated_ = !decorated;
  frame->transparent_ = transparent;
  return frame;
}
...
+ (J2ObjcClassInfo *)__metadata {
  static J2ObjcMethodInfo methods[] = {
    ...
    { "createFrameWithRAREiWidget:withRAREaWindowManager_WindowTypeEnum:withBoolean:withBoolean:withBoolean:", NULL, "LRAREiWindowManager_iFrame", 0x4, NULL },
    { "setWindowIconsExWithJavaUtilList:", NULL, "V", 0x4, NULL },
  };
  static J2ObjcClassInfo _RAREWindowManager = { "WindowManager", "com.appnativa.rare.ui", NULL, 0x1, 7, methods, 0, NULL, 0, NULL};
