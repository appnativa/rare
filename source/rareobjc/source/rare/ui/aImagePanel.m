...
- (int)getImageScaleWidth {
  return (theImage_ == nil) ? 0 : (int) ((RAREUIRectangle *) nil_chk(destBounds_))->width_;
}

- (int)getImageScaleHeight {
  return (theImage_ == nil) ? 0 : (int) ((RAREUIRectangle *) nil_chk(destBounds_))->height_;
}
...
- (RAREUIImage *)getSubImageWithRAREiPlatformShape:(id<RAREiPlatformShape>)shape {
  if (![((RAREUIImage *) nil_chk(theImage_)) isImageLoadedWithRAREiImageObserver:self]) {
    return nil;
  }
...
- (void)init__WithInt:(int)iw
  ...
  BOOL loaded = (theImage_ != nil) && [theImage_ isImageLoadedWithRAREiImageObserver:self];
  if (reset) {
    [self initValues];
  }
  int imageWidth = iw;
...
- (BOOL)isImageFitted {
  if ((((RAREUIRectangle *) nil_chk(destBounds_))->x_ > -1) && (destBounds_->y_ > -1)) {
    ...
    if (rotation_ != 0) {
      return ((int) destBounds_->width_ <= h) && ((int) destBounds_->height_ <= w);
    }
    return ((int) destBounds_->width_ <= w) && ((int) destBounds_->height_ <= h);
  }
  return NO;
}
