...
+ (int)findNextSelectableWithRAREiListHandler:(id<RAREiListHandler>)comp
  ...
  for (int i = start; i < len; i++) {
    ...
    if ((di != nil) && [di isSelectable]) {
      if ([comp isTabular]) {
        id<RAREiWidget> w = [((id<RAREiPlatformComponent>) nil_chk([comp getListComponent])) getWidget];
        if ([(id) w isKindOfClass:[RARETableViewer class]]) {
          RARETableViewer *table = (RARETableViewer *) check_class_cast(w, [RARETableViewer class]);
          id<RAREiTreeHandler> th = [((RARETableViewer *) nil_chk(table)) getTreeHandler];
          if (th != nil && ![th isParentItemsSelectable] && ![th isLeafItemWithInt:i]) {
            continue;
          }
        }
      }
...
+ (void)selectNextRowWithRAREiListHandler:(id<RAREiListHandler>)comp
                                  withInt:(int)row {
  int pos = row == -1 ? 0 : [RAREUISelectionModelGroup findNextSelectableWithRAREiListHandler:comp withInt:row + 1 withInt:-1];
  if (pos != -1) {
...
+ (void)selectPreviousRowWithRAREiListHandler:(id<RAREiListHandler>)comp
                                      withInt:(int)row {
  int pos = row == -1 ? [((id<RAREiListHandler>) nil_chk(comp)) getItemCount] - 1 : [RAREUISelectionModelGroup findPreviousSelectableWithRAREiListHandler:comp withInt:0 withInt:row];
  if (pos != -1) {
    [((id<RAREiListHandler>) nil_chk(comp)) setSelectedIndexWithInt:pos];
    [comp scrollRowToVisibleWithInt:pos];
  }
