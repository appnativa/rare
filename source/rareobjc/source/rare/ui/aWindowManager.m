...
- (void)configureStandardStuffWithRARESPOTMainWindow:(RARESPOTMainWindow *)cfg {
  ...
  if ([((SPOTPrintableString *) nil_chk(cfg->bgColor_)) spot_hasValue]) {
    ...
    if (windowPaintbucket_ != nil) {
      [windowPaintbucket_ installWithRAREiPlatformComponent:[((id<RAREiWindowManager_iFrame>) nil_chk(mainFrame_)) getComponent]];
    }
  }
  else if ([((SPOTBoolean *) nil_chk(cfg->decorated_)) booleanValue]) {
    [((id<RAREiPlatformComponent>) nil_chk([((id<RAREiWindowManager_iFrame>) nil_chk(mainFrame_)) getComponent])) setBackgroundWithRAREUIColor:[RAREColorUtils getBackground]];
  }
