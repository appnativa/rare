...
+ (void)initilizeWithFloat:(float)sdensity
  ...
  [RAREConstantSize setPixelMultiplierWithFloat:pmultiplier];
}
...
+ (int)toPlatformPixelHeightWithFloat:(float)h
  ...
  switch ([unit ordinal]) {
    ...
    case RAREScreenUtils_Unit_DIALOG:
    h = [RARESizes dialogUnitYAsPixelWithInt:(int) h withRAREiPlatformComponent:c];
    break;
    default:
...
+ (int)toPlatformPixelWidthWithFloat:(float)w
  ...
  switch ([unit ordinal]) {
    ...
    case RAREScreenUtils_Unit_DIALOG:
    w = [RARESizes dialogUnitXAsPixelWithInt:(int) w withRAREiPlatformComponent:c];
    break;
    default:
...
+ (float)getDefaultIconDensity {
  if (RAREScreenUtils_iconDensity_ == 0) {
    NSString *s = [((RAREUIProperties *) nil_chk([RAREPlatform getUIDefaults])) getStringWithNSString:@"Rare.defaultIconDensity"];
    do {
      if (s == nil) {
        break;
      }
...
+ (J2ObjcClassInfo *)__metadata {
  static J2ObjcMethodInfo methods[] = {
    ...
    { "calculateScreenSize", NULL, "V", 0x8, NULL },
  };
