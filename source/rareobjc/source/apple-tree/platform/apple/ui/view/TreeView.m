...
//  source: /Code/Dev/appNativa/source/rareobjc/../rare/apple-tree/com/appnativa/rare/platform/apple/ui/view/TreeView.java
//
...
- (void)getPreferredSizeWithRAREUIDimension:(RAREUIDimension *)size
  ...
  if ([self isTable]) {
    [super getPreferredSizeWithRAREUIDimension:size withInt:maxWidth];
    return;
  }
...
- (void)itemSelectedWithInt:(int)index {
  if (!toggleOnTwistyOnly_) {
    RARERenderableDataItem *item = [((id<RAREiPlatformListDataModel>) nil_chk(listModel_)) getWithInt:index];
    id<RAREiTreeItem> ti = [self getTreeItemWithRARERenderableDataItem:item];
    if ((ti != nil) && ![ti isLeaf]) {
      [self handleExpansionWithRAREiTreeItem:ti withInt:index withBoolean:YES];
    }
  }
...
- (void)loadIcons {
  if (collapsedIcon_ == nil) {
    collapsedIcon_ = [((RAREUIProperties *) nil_chk([RAREPlatform getUIDefaults])) getIconWithNSString:@"Rare.Tree.collapsedIcon"];
  }
  if (expandedIcon_ == nil) {
    expandedIcon_ = [((RAREUIProperties *) nil_chk([RAREPlatform getUIDefaults])) getIconWithNSString:@"Rare.Tree.expandedIcon"];
  }
...
- (void)updateRenderInsetsForCheckBoxWithInt:(int)left
                                     withInt:(int)right {
  if (itemRenderer_ != nil) {
    RAREUIInsets *in = [itemRenderer_ getInsets];
    [((RAREUIInsets *) nil_chk(in)) setWithRAREUIInsets:rinsets_];
    in->left_ += left;
    in->right_ += right;
  }
}
...
+ (J2ObjcClassInfo *)__metadata {
  static J2ObjcMethodInfo methods[] = {
    ...
    { "updateRenderInsetsForCheckBoxWithInt:withInt:", NULL, "V", 0x4, NULL },
  ...
  static J2ObjcClassInfo _RARETreeView = { "TreeView", "com.appnativa.rare.platform.apple.ui.view", NULL, 0x1, 27, methods, 19, fields, 0, NULL};
