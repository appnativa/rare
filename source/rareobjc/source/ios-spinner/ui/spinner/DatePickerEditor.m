...
- (void)setValueWithId:(id)value {
  ...
  if ([value isKindOfClass:[JavaUtilCalendar class]]) {
    d = [((JavaUtilCalendar *) check_class_cast(value, [JavaUtilCalendar class])) getTime];
  }
  else if ([value isKindOfClass:[JavaUtilDate class]]) {
    d = (JavaUtilDate *) check_class_cast(value, [JavaUtilDate class]);
  }
  if (d == nil) {
    if (min_ != nil) {
      d = min_;
      ...
      d = [[JavaUtilDate alloc] init];
    }
  }
  [((JavaUtilCalendar *) nil_chk(date_)) setTimeWithJavaUtilDate:d];
  [((RAREDatePickerView *) check_class_cast(editorView_, [RAREDatePickerView class])) setDateWithJavaUtilCalendar:date_];
  [((id<RAREiSpinnerModel>) nil_chk(spinnerModel_)) setValueWithId:date_];
}

- (id)getValue {
  return [((JavaUtilCalendar *) nil_chk(date_)) getTime];
}
