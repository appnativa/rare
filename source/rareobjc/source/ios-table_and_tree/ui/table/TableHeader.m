...
- (void)setHeaderCellPainterWithRAREPaintBucket:(RAREPaintBucket *)headerPainter {
  [super setHeaderCellPainterWithRAREPaintBucket:headerPainter];
  [((RARETableHeader_TableHeaderView *) nil_chk(headerView_)) setComponentPainterWithRAREiPlatformComponentPainter:(headerPainter == nil) ? nil : [headerPainter getCachedComponentPainter]];
}
...
- (id)initWithRARETableView:(RARETableView *)table
                     withId:(id)proxy {
  if (self = [self initRARETableHeader_TableHeaderViewWithRARETableView:table withId:proxy withId:proxy]) {
    usePainterBorder_ = NO;
  }
  return self;
}
...
- (void)paintBackgroundWithRAREAppleGraphics:(RAREAppleGraphics *)g
  ...
  if (header_->showHeaderMargin_) {
    [self drawSeparatorExWithRAREAppleGraphics:g withRAREiPlatformBorder:b withRAREUIColor:header_->marginColor_];
