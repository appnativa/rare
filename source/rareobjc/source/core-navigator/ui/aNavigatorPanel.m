...
- (void)layoutWithFloat:(float)width
  ...
  for (int i = 0; i < len; i++) {
    ...
    count++;
    RARECellConstraints *cc = [RAREaComponent getMeasuredCellConstraintsWithRAREiPlatformComponent:c withBoolean:NO];
  ...
  if (buttonsSameSize_) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      RARECellConstraints *cc = [RAREaComponent getMeasuredCellConstraintsWithRAREiPlatformComponent:c withBoolean:NO];
      w = ((RARECellConstraints *) nil_chk(cc))->gridWidth_ + dx;
...
- (void)getPreferredSizeExWithRAREUIDimension:(RAREUIDimension *)size
  ...
  for (int i = 0; i < len; i++) {
    ...
    count++;
    RARECellConstraints *cc = [RAREaComponent getMeasuredCellConstraintsWithRAREiPlatformComponent:c withBoolean:NO];
    width += ((RARECellConstraints *) nil_chk(cc))->gridWidth_ + p1;
...
- (void)initialize__WithNSString:(NSString *)text
  ...
  if (a != nil) {
    ...
    if ([(id) selectedIcon_ conformsToProtocol: @protocol(RAREiObservableImage)]) {
      [((id<RAREiObservableImage>) check_protocol_cast(selectedIcon_, @protocol(RAREiObservableImage))) isImageLoadedWithRAREiImageObserver:this$0_];
    }
    if ([(id) [a getIcon] conformsToProtocol: @protocol(RAREiObservableImage)]) {
      [((id<RAREiObservableImage>) check_protocol_cast([a getIcon], @protocol(RAREiObservableImage))) isImageLoadedWithRAREiImageObserver:this$0_];
    }
    action_ = a;
  }
  if (text != nil) {
  ...
  if (icon != nil) {
    if ([(id) icon conformsToProtocol: @protocol(RAREiObservableImage)]) {
      [((id<RAREiObservableImage>) check_protocol_cast(icon, @protocol(RAREiObservableImage))) isImageLoadedWithRAREiImageObserver:this$0_];
    }
