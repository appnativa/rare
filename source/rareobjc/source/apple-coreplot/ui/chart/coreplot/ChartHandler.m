...
- (void)getPreferredSizeExWithRAREUIDimension:(RAREUIDimension *)size
  ...
  if ((legendCount_ > 0) && (maxWidth > 0)) {
    int cols = (int) [JavaLangMath floorWithDouble:(maxWidth + legendMaxWidth_ - 5) / legendMaxWidth_];
    int rows = (int) [JavaLangMath floorWithDouble:(legendCount_ + cols - 1) / cols];
    ((RAREUIDimension *) nil_chk(size))->width_ = cols * legendMaxWidth_;
    size->height_ = rows & legendHeight_;
  }
  else {
    if (horizontal_) {
