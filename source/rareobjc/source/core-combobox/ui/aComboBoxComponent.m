...
- (void)cancelPopup {
  if ([self isPopupVisible]) {
    [self popupMenuCanceledWithRAREExpansionEvent:[[RAREExpansionEvent alloc] initWithId:currentPopup_]];
  }
}
...
- (void)configurationCompletedWithRAREaWidget:(RAREaWidget *)w
  ...
  if (popupPainter_ == nil) {
    ...
    if (isDefaultContent_) {
      RAREUIColor *bg = [((id<RAREiPlatformComponent>) nil_chk([((id<RAREiPlatformListHandler>) nil_chk(listHandler_)) getContainerComponent])) getBackground];
      if (bg == nil) {
      ...
      [defaultPopupPainter_ setBackgroundColorWithRAREUIColor:bg];
    ...
    if ([defaultPopupPainter_ getBorder] == nil) {
      ...
      if (usingDefaultBorder_) {
        ...
        if (([cfg isKindOfClass:[RARESPOTComboBox class]]) && [((SPOTBoolean *) nil_chk(((RARESPOTComboBox *) nil_chk(cfg))->useSameBorderForPopup_)) booleanValue]) {
...
- (void)getPreferredSizeExWithRAREUIDimension:(RAREUIDimension *)size
  ...
  if ((standardBorder_ != nil) && [RAREPlatform isTouchDevice]) {
    height += [RAREScreenUtils PLATFORM_PIXELS_6];
  }
...
- (void)hidePopupExWithBoolean:(BOOL)callhideOnPopup {
  if (currentPopup_ != nil) {
    ...
    if (callhideOnPopup && (listenerList_ != nil)) {
      [RAREUtils firePopupEventWithRAREEventListenerList:listenerList_ withRAREExpansionEvent:[[RAREExpansionEvent alloc] initWithId:self] withBoolean:NO];
    }
    currentPopup_ = nil;
...
- (void)paintIconWithRAREiPlatformGraphics:(id<RAREiPlatformGraphics>)graphics {
  if (editorIcon_ != nil) {
    ...
    [editorIcon_ paintWithRAREiPlatformGraphics:graphics withFloat:x withFloat:([self getHeight] - [editorIcon_ getIconHeight]) / 2 withFloat:[editorIcon_ getIconWidth] withFloat:[editorIcon_ getIconHeight]];
  }
  if ([self isButtonVisible]) {
    ...
    if (fg == nil) {
      fg = [down isEnabled] ? [RAREUIColorHelper getForeground] : [RAREUIColorHelper getDisabledForeground];
    }
    if (icon == nil) {
      icon = downIcon_;
    }
...
- (void)popupActionPerformedWithRAREActionEvent:(RAREActionEvent *)e {
  if (listHandler_ != nil) {
    RARERenderableDataItem *item = [listHandler_ getSelectedItem];
    if ((item != nil) && ([item getActionListener] != nil)) {
      [((id<RAREiActionListener>) nil_chk([item getActionListener])) actionPerformedWithRAREActionEvent:[[RAREActionEvent alloc] initWithId:self]];
      return;
    }
  }
  if (listenerList_ != nil) {
...
- (void)showPopupEx {
  ...
  if (standardBorder_ != nil) {
    ...
    [self setBorderWithRAREiPlatformBorder:standardBorder_];
    [self repaint];
    [RAREPlatform invokeLaterWithJavaLangRunnable:[[RAREaComboBoxComponent_$1 alloc] initWithRAREaComboBoxComponent:self withRAREiPopup:p]];
  }
  else {
    currentPopup_ = p;
...
- (void)focusChangedWithId:(id)view
  ...
  [this$0_ focusChangedWithId:view withBoolean:hasFocus withId:oppositeView];
}
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _RAREaComboBoxComponent_$1 = { "$1", "com.appnativa.rare.ui", "aComboBoxComponent", 0x8000, 0, NULL, 2, fields, 0, NULL};
