...
- (int)addTabWithNSString:(NSString *)name
  ...
  [((id<RAREiTabDocument>) nil_chk(doc)) setActionLinkWithRAREActionLink:[[RAREActionLink alloc] initWithRAREUTiURLResolver:self withJavaNetURL:url withNSString:nil]];
  if (load_) {
...
+ (void)configureTabOptionsWithRARESPOTTabPane:(RARESPOTTabPane *)tc
  ...
  tabOptions->tabColorHolder_ = [RAREColorUtils configureWithRAREiWidget:context withRARESPOTGridCell:[tc getTabPainter] withRAREPaintBucket:nil];
  tabOptions->selectedTabColorHolder_ = [RAREColorUtils configureWithRAREiWidget:context withRARESPOTGridCell:[tc getSelectedTabPainter] withRAREPaintBucket:nil];
...
+ (void)configureTabVisualsWithRAREiViewer:(id<RAREiViewer>)context
  ...
  if (tab != nil) {
    ...
    if ([((SPOTPrintableString *) nil_chk(tab->bgColor_)) getValue] != nil) {
      ...
      [RAREColorUtils configureBackgroundPainterWithRAREPaintBucket:colorHolder withSPOTPrintableString:tab->bgColor_];
    }
  }
  if (colorHolder != nil) {
...
- (void)register__ {
  [super register__];
  id<RAREiPlatformWindowManager> wm = [((id<RAREiPlatformAppContext>) nil_chk([self getAppContext])) getWindowManager];
  int len = [((id<RAREiTabPaneComponent>) nil_chk(tabPane_)) getTabCount];
  for (int i = 0; i < len; i++) {
    id<RAREiTabDocument> doc = [tabPane_ getDocumentAtWithInt:i];
    id<RAREiTarget> t = [((id<RAREiTabDocument>) nil_chk(doc)) getTarget];
    [((id<RAREiPlatformWindowManager>) nil_chk(wm)) registerTargetWithNSString:[((id<RAREiTarget>) nil_chk(t)) getName] withRAREiTarget:t];
...
- (id<RAREiTabDocument>)createDocumentWithRAREiTabPaneViewer:(id<RAREiTabPaneViewer>)tpv
  ...
  if (name == nil) {
    ...
    if (name == nil) {
      name = [NSString stringWithFormat:@"TabPane_%@", [JavaLangInteger toHexStringWithInt:(int) [tpv hash]]];
    }
