...
- (void)moveWithInt:(int)from
            withInt:(int)to {
  if (![RAREUTIntList moveWithIntArray:A_ withInt:_length_ withInt:from withInt:to]) {
...
+ (BOOL)moveWithIntArray:(IOSIntArray *)list
                 ...
                 withInt:(int)from
                 withInt:(int)to {
  int len = length == -1 ? (int) [((IOSIntArray *) nil_chk(list)) count] : length;
  if (to < 0 || to >= len) {
    return NO;
  }
  if (from < 0 || from >= len) {
  ...
  int val = IOSIntArray_Get(nil_chk(list), from);
  if (from != len - 1) {
  ...
  (*IOSIntArray_GetRef(list, to)) = val;
  return YES;
}
...
+ (J2ObjcClassInfo *)__metadata {
  static J2ObjcMethodInfo methods[] = {
    ...
    { "moveWithIntArray:withInt:withInt:withInt:", NULL, "Z", 0x9, NULL },
  ...
  static J2ObjcClassInfo _RAREUTIntList = { "IntList", "com.appnativa.util", NULL, 0x11, 10, methods, 2, fields, 0, NULL};
