...
- (NSString *)description {
  ...
  if (msize > 0) {
    [RAREUTSNumber getCharsWithLong:mantissa_ withInt:msize withCharArray:out->A_];
  }
  if (fsize > 0) {
    if ((mantissa_ == 0) && (fraction_ < 0)) {
      (*IOSCharArray_GetRef(nil_chk(out->A_), msize++)) = RAREUTSNumber_MINUS_SIGN_;
    }
    (*IOSCharArray_GetRef(nil_chk(out->A_), msize)) = RAREUTSNumber_DECIMAL_POINT_;
    if (cl > 0) {
      int i = 0;
      while (i < cl) {
        (*IOSCharArray_GetRef(out->A_, ++msize)) = RAREUTSNumber_ZERO_DIGIT_;
        i++;
      }
    }
    out->_length_ = fsize + msize + 1;
    [RAREUTSNumber getCharsWithLong:frac withInt:out->_length_ withCharArray:out->A_];
    if (cl < 0) {
      out->_length_ += cl;
    }
  }
