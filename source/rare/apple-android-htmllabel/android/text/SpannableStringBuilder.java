...
public class SpannableStringBuilder
  ...
  /**
   * Create a new SpannableStringBuilder containing a copy of the specified text, including its
   * spans if any.
   */
  ...
  /**
   * Create a new SpannableStringBuilder containing a copy of the specified slice of the specified
   * text, including its spans if any.
   */
  ...
  /**
   * Return the buffer offset of the beginning of the specified markup object, or -1 if it is not
  ...
  /**
   * Return the buffer offset of the end of the specified markup object, or -1 if it is not attached
   * to this buffer.
   */
  @Override
  ...
  /**
   * Return the flags of the end of the specified markup object, or 0 if it is not attached to this
   * buffer.
   */
  @Override
  ...
  /**
   * Return an array of the spans of the specified type that overlap the specified range of the
   * buffer. The kind may be Object.class to get a list of all the spans regardless of type.
   */
  @Override
   ...
   * <code>start</code> but less than or equal to
   * <code>limit</code> where a span of the specified type begins or ends.
   */
  @Override
  ...
  /**
   * Return a new CharSequence containing a copy of the specified range of this buffer, including
  ...
  /**
   * Copy the specified range of chars from this buffer into the specified array, beginning at the
   * specified offset.
   */
  @Override
...
//  }
  ...
  /*
   * private static final int startFlag(int flag) { return (flag >> 4) & 0x0F; }
   *
   * private static final int endFlag(int flag) { return flag & 0x0F; }
   *
   * public void dump() { // XXX for (int i = 0; i < mGapStart; i++) { System.out.print('|');
   * System.out.print(' '); System.out.print(isprint(mText[i]) ? mText[i] : '.'); System.out.print('
   * '); }
   *
   * for (int i = mGapStart; i < mGapStart + mGapLength; i++) { System.out.print('|');
   * System.out.print('('); System.out.print(isprint(mText[i]) ? mText[i] : '.');
   * System.out.print(')'); }
   *
   * for (int i = mGapStart + mGapLength; i < mText.length; i++) { System.out.print('|');
   * System.out.print(' '); System.out.print(isprint(mText[i]) ? mText[i] : '.'); System.out.print('
   * '); }
   *
   * System.out.print('\n');
   *
   * for (int i = 0; i < mText.length + 1; i++) { int found = 0; int wfound = 0;
   *
   * for (int j = 0; j < mSpanCount; j++) { if (mSpanStarts[j] == i) { found = 1; wfound = j; break;
   * }
   *
   * if (mSpanEnds[j] == i) { found = 2; wfound = j; break; } }
   *
   * if (found == 1) { if (startFlag(mSpanFlags[wfound]) == MARK) System.out.print("( "); if
   * (startFlag(mSpanFlags[wfound]) == PARAGRAPH) System.out.print("< "); else System.out.print("[
   * "); } else if (found == 2) { if (endFlag(mSpanFlags[wfound]) == POINT) System.out.print(") ");
   * if (endFlag(mSpanFlags[wfound]) == PARAGRAPH) System.out.print("> "); else System.out.print("]
...
//    private InputFilter[] mFilters = NO_FILTERS;
  ...
  public void dispose() {
   	if(mSpans!=null) {
   		for(int i=0;i<mSpans.length;i++) {
   		  if(mSpans[i] instanceof Object[]) {
   		    cleanArray((Object[]) mSpans[i]);
   		  }
     		mSpans[i]=null;
   		}
   		mSpans=null;
  ...
  private void cleanArray(Object[] a) {
    int len=a.length;
    for(int i=0;i<len;i++) {
      a[i]=null;
    }
