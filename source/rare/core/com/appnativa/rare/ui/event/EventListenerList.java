...
public class EventListenerList implements Serializable {
  ...
  public synchronized <T> void add(final Class<T> listenerClass, final T listener) {
    if (listener == null) {
      return;
    }
    ...
    for (int i = listenerList.length - 1; i > 0; i -= 2) {
      if ((listenerClass == listenerList[i - 1]) && (listener == listenerList[i])) {
        position = i - 1;
  ...
  public synchronized <T> void remove(final Class<T> listenerClass, final T listener) {
    if (listener == null) {
      return;
    }
  ...
  public synchronized <T> boolean hasListener(final Class<T> listenerClass, final T listener) {
    if (listener == null) {
      return false;
    }
  ...
  public boolean hasListeners(final Class<?> listenerClass) {
    return getListenerCount(listenerClass) > 0;
  }
